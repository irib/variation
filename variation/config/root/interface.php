...
/*
    ...
    */
	...
	var element = $("#saved_msg");
	if(element.length){
	  //L'élément existe
	  $("#saved_msg").remove();
	}
	
	$("#tsm_save").parent().append('<span id="saved_msg">Patientez...</span>');
	$.post('/ajax/root_interface/topssmenu_update.php', { tsm_id: displayed_tsm, titre: titre, icone: icone, type: type, type_id: type_id, sme_id_lien_usager: tsm_sme_id },function(){
		
		$("#saved_msg").html("Modifications enregistrées");
		$("#saved_msg").stop().delay(2000).fadeOut(900,function(){$('#saved_msg').remove()});
	});
    });
    ...
    $("#tsm_type_affectations_id").change (function () {
	affiche_aff ($(this).val());
    });
    $("#tsm_type_histoaffectations_id").change (function () {
	affiche_haf ($(this).val());
    });
	...
	$("#lis_save").parent().append('<span id="saved_msg">Patientez...</span>');
	...
	$("#dos_save").parent().append('<span id="saved_msg">Patientez...</span>');
	...
	$("#nos_save").parent().append('<span id="saved_msg">Patientez...</span>');
    ...
    $("#aff_save").click (function () {
	var code = $("#aff_code").val();
	var titre = $("#aff_titre").val();
	var champ = $("#aff_champ").val();
	var element = $("#saved_msg");
	if(element.length){
	  //L'élément existe
	  $("#saved_msg").remove();
	}
	$("#aff_save").parent().append('<span id="saved_msg">Patientez...</span>');
	
	
	$.post('/ajax/root_interface/affectation_update.php', { aff_id: displayed_aff_id, titre: titre, code: code, champ: champ }, function () {
	    tsm_type_affectations_id_update ();
	    load_tsm (displayed_tsm);
		$("#saved_msg").html("Modifications enregistrées");
		$("#saved_msg").stop().delay(2000).fadeOut(900,function(){$('#saved_msg').remove()});
    ...
    $("#aff_supprime").click (function () {
	$.post('/ajax/root_interface/affectation_supprime.php', { aff_id: displayed_aff_id }, function () {
	    tsm_type_affectations_id_update ();
	    load_tsm (displayed_tsm);
	});
    });

    $("#haf_save").click (function () {
	var code = $("#haf_code").val();
	var titre = $("#haf_titre").val();
	var champ = $("#haf_champ").val();
	var element = $("#saved_msg");
	if(element.length){
	  //L'élément existe
	  $("#saved_msg").remove();
	}
	$("#haf_save").parent().append('<span id="saved_msg">Patientez...</span>');
	
	
	$.post('/ajax/root_interface/histoaffectation_update.php', { haf_id: displayed_haf_id, titre: titre, code: code, champ: champ }, function () {
	    tsm_type_histoaffectations_id_update ();
	    load_tsm (displayed_tsm);
		$("#saved_msg").html("Modifications enregistrées");
		$("#saved_msg").stop().delay(2000).fadeOut(900,function(){$('#saved_msg').remove()});
    ...
    $("#haf_supprime").click (function () {
	$.post('/ajax/root_interface/histoaffectation_supprime.php', { haf_id: displayed_haf_id }, function () {
	    tsm_type_histoaffectations_id_update ();
	    load_tsm (displayed_tsm);
	});
    });
...
function tsm_type_affectations_id_update () {
    $.getJSON ('/ajax/root_interface/affectations_liste.php', {}, function (data) {
	$("#tsm_type_affectations_id").empty ().append ('<option value=""></option>');
	$.each (data, function (key, val) {
	    $("#tsm_type_affectations_id").append ('<option value="'+val.aff_id+'">'+val.aff_titre+'</option>');
	});
    });
}

function tsm_type_histoaffectations_id_update () {
    $.getJSON ('/ajax/root_interface/histoaffectations_liste.php', {}, function (data) {
	$("#tsm_type_histoaffectations_id").empty ().append ('<option value=""></option>');
	$.each (data, function (key, val) {
	    $("#tsm_type_histoaffectations_id").append ('<option value="'+val.haf_id+'">'+val.haf_titre+'</option>');
	});
    });
}
...
//    $("#outils").hide();
	...
	} else if (data.tsm_type == 'affectations') {
	    $("#tsm_type_affectations_id").val (data.tsm_type_id);
	    var aff_id = data.tsm_type_id;
	    affiche_aff (aff_id);	    
	} else if (data.tsm_type == 'histoaffectations') {
	    $("#tsm_type_histoaffectations_id").val (data.tsm_type_id);
	    var haf_id = data.tsm_type_id;
	    affiche_haf (haf_id);	    
	}
...
var displayed_aff_id = null;
function affiche_aff (aff_id) {
  $.getJSON ('/ajax/root_interface/affectations_get.php', { aff_id: aff_id }, function (data) {
	displayed_aff_id = aff_id;
	$("#aff_titre").val (data.aff_titre);
	$("#aff_code").val (data.aff_code);
	$("#aff_champ").val (data.inf_id);
    });

}

var displayed_haf_id = null;
function affiche_haf (haf_id) {
  $.getJSON ('/ajax/root_interface/histoaffectations_get.php', { haf_id: haf_id }, function (data) {
	displayed_haf_id = haf_id;
	$("#haf_titre").val (data.haf_titre);
	$("#haf_code").val (data.haf_code);
	$("#haf_champ").val (data.inf_id);
    });
...
var colle_sme_id = null; // Sous-menu masque de données à coller
function getItems (node) {
	...
	items.purger_por = {
	    ...
	    "action": function () {
	      if (confirm ('Vous allez purger ce portail')) {
	...
	items.dupliquer_por = {
	    "label" : "Dupliquer portail",
	    "action": function () {
	      if ((nom = prompt ('Nom du nouveau portail'))) {
		$.post('/ajax/root_interface/portail_dupliquer.php', { por: node.attr('id'), nom: nom }, function () {
		    $("#tree").jstree ('refresh', -1);
		  });
	      }
	    }
	...
	items.supprimer = {
	    ...
	    "action": function () {
			if (confirm('Vous allez définitivement supprimer ce menu et tout ce qu\'il contient. Continuer ?')){
	...
	items.supprimer = {
	    "label" : "Supprimer menu",
	    "action": function () {
		if (confirm('Vous allez définitivement supprimer ce menu. Continuer ?')){
	...
	items.supprimer = {
	    ...
	    "action": function () {
		if(confirm("Vous allez définitivement supprimer cette fiche. Continuer ?")){
	...
	var subtype = node.attr('sub');
	if (subtype == undefined) {
	  items.copier = {
	    "label": "Copier les champs",
	    "action": function() {
	      colle_sme_id = node.attr('id');
	    }
	  };
	  items.coller = {
	    "label": "Coller les champs",
	    "action": function() {
	      if (colle_sme_id == null) {
		alert('Vous devez d\'abord copier une fiche masque de données');
		return;
	      }
	      $.post('/ajax/root_interface/sme_masque_colle.php', 
	    { dst: node.attr('id'), src: colle_sme_id }, 
		     function () {
		       colle_sme_id = null;
...
function getCol3Items (node) {
	...
	items.supprimer = {
	  "label": "Supprimer",
	  "action": function () {
		if(confirm("Vous allez définitivement supprimer cela. Continuer ?")){
			var din = node.attr('id');
	...
	"label": "Supprimer",
	"action": function () {
	  if (confirm ('Vous allez définitivement supprimer ce champ. Continuer ?')) {
	    $.post('/ajax/root_interface/info_supprime.php', { prm_id: node.attr('id') }, function (res) {
		if (res == '0') {
		  alert('Erreur lors de la suppression. Veuillez vérifier qu\'il n\'est pas utilisé avant de le supprimer.');
		} else {
		  $("#listechamps").jstree ('refresh', -1);
		}
	      });
...
function info_usage (id) {
  $.getJSON('/ajax/meta_info_usage.php', { prm_token: $("#token").val(), output: 'json2', prm_inf_id: id.substr (4) }, function (data) {
      ...
      if (data) {
	$.each (data, function (key, val) {
	    txt += val.cat_nom+' > '+val.por_libelle;
	    if (val.ent_libelle != null)
	     txt += ' > '+val.ent_libelle+' > '+val.men_libelle+' > '+val.sme_libelle;
	    txt += '\n';
	  });
	alert (txt);
      } else {
...
<select id="tsm_type">
  ...
  <option value="notes">Notes</option>
  <option value="affectations">Affectations aux groupes</option>
  <option value="histoaffectations">Historique des affectations</option>
</select><br/>
...
<div class="tr_type tr_type_affectations">
Vue d'affectation : <br/>
<select id="tsm_type_affectations_id"><option value=""></option><?= affiche_liste_affectations (); ?></select>
</div>

<div class="tr_type tr_type_histoaffectations">
Vue d'historique d'affectation : <br/>
<select id="tsm_type_histoaffectations_id"><option value=""></option><?= affiche_liste_histoaffectations (); ?></select>
</div>
...
<div class="col2bloc tr_details tr_details_affectations">
<h2>Vue d'affectations aux groupes</h2>

Titre :<br/>
<input type="text" size="30" id="aff_titre"></input><br/>

Code :<br/>
<input type="text" size="30" id="aff_code"></input><br/>

Champs d'affectation :<br/>
<select id="aff_champ"><option value=""></option><?= affiche_champs_affectation() ?></select><br/>

<button id="aff_save">Enregistrer</button>
<button id="aff_supprime">Supprimer</button>

</div>


<div class="col2bloc tr_details tr_details_histoaffectations">
<h2>Vue d'historique d'affectations aux groupes</h2>

Titre :<br/>
<input type="text" size="30" id="haf_titre"></input><br/>

Code :<br/>
<input type="text" size="30" id="haf_code"></input><br/>

Champs d'affectation :<br/>
<select id="haf_champ"><option value=""></option><?= affiche_champs_affectation() ?></select><br/>

<button id="haf_save">Enregistrer</button>
<button id="haf_supprime">Supprimer</button>
...
function affiche_liste_affectations () {
  global $base;
  $affs = $base->vues_affectations_liste ($_SESSION['token']);
  foreach ($affs as $aff) {
    echo '<option value="'.$aff['aff_id'].'">'.$aff['aff_titre'].'</option>';
  }
}

function affiche_liste_histoaffectations () {
  global $base;
  $hafs = $base->vues_histoaffectations_liste ($_SESSION['token']);
  foreach ($hafs as $haf) {
    echo '<option value="'.$haf['haf_id'].'">'.$haf['haf_titre'].'</option>';
  }
}
...
function affiche_champs_affectation() {
  global $base;
  $infs = $base->meta_info_liste($_SESSION['token'], NULL, TRUE, 'groupe');
  foreach ($infs as $inf) {
    echo '<option value="'.$inf['inf_id'].'">'.$inf['inf_libelle'].'</option>';
  }
}
?>
