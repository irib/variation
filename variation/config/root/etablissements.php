...
$pagination->setFunction ('etablissement_liste_details', array ($_SESSION['token'], arrval ($_GET, 'sec_id'), TRUE));
$pagination->setUrl ('/admin.php', array ('sec_id' => arrval ($_GET, 'sec_id') ));
...
if (isset ($_GET['eta'])) {
  ...
  if (count ($erreur)) {
    ...
    $etabli['eta_id'] = isset ($eta_id) ? $eta_id : NULL;
    $etabli['cat_id'] = $_POST['ed_cat'];
    $etabli['eta_nom'] = $_POST['ed_intitule'];
    if (isset ($_POST['ed_secteur']) && count ($_POST['ed_secteur'])) {
      foreach ($_POST['ed_secteur'] as $s) {
	$etablissement_secteurs[] = array ('sec_code' => $s);
      }
    }
...
function on_ed_suppr_click () {
    if (confirm ('Supprimer l\'organisme "<?= addslashes (arrval ($etabli, 'eta_nom', '')) ?>" ?')) {
...
<option value="">(toutes thématiques)</option>
  <?php liste_secteurs (arrval ($_GET, 'sec_id')); ?>
</select>
...
<?php } ?>
  <tr class="pair">
    <td>Catégorie</td>
     <td><select name="ed_cat"><option value="">(choisir une catégorie)</option><?php liste_categories (arrval ($etabli, 'cat_id')) ?></select><?= arrval ($erreur, 'ed_cat', '') ?></td>
  </tr>

  <tr class="impair">
    <td>Intitulé</td>
     <td><input size="30" name="ed_intitule" value="<?= arrval ($etabli, 'eta_nom', '') ?>"></input><?= arrval ($erreur, 'ed_intitule', '') ?></td>
  </tr>

  <tr class="pair">
    <td>Adresse</td>
     <td><textarea name="ed_adr_adresse" rows="3" cols="30"><?= arrval ($etab_adresse, 'adr_adresse', '') ?></textarea></td>
  </tr>

  <tr class="impair">
    <td>CP/Ville</td>
    <td>
      <input type="text" size="5" name="ed_adr_cp" value="<?= arrval ($etab_adresse, 'adr_cp', '') ?>"></input>
      <input type="text" size="20" name="ed_adr_ville" value="<?= arrval ($etab_adresse, 'adr_ville', '') ?>"></input>
    </td>
  </tr>
  ...
  <tr class="pair">
    <td>Tél/Fax</td>
    <td>
      <input type="text" size="12" name="ed_adr_tel" value="<?= arrval ($etab_adresse, 'adr_tel', '') ?>"></input>
      <input type="text" size="12" name="ed_adr_fax" value="<?= arrval ($etab_adresse, 'adr_fax', '') ?>"></input>
    </td>
  </tr>
  ...
  <tr class="impair">
    <td>Email</td>
    <td>
      <input type="text" size="30" name="ed_adr_email" value="<?= arrval ($etab_adresse, 'adr_email', '') ?>"></input>
    </td>
  </tr>
  ...
  <tr class="pair">
    <td>Site web</td>
    <td>
      <input type="text" size="30" name="ed_adr_web" value="<?= arrval ($etab_adresse, 'adr_web', '') ?>"></input>
    </td>
  </tr>
...
function lien_edit_etab ($eta_id) {
  return array ('/admin.php', array ('sec_id' => arrval ($_GET, 'sec_id'),
				     'eta' => $eta_id));
}
...
function etablissement_save ($post) {
  ...
  $secteurs = array ();
  if (isset ($post['ed_secteur']) && count ($post['ed_secteur'])) {
    foreach ($post['ed_secteur'] as $secteur) {
      $secteurs[] = $secteur;
    }
  }
  $base->etablissement_secteurs_set ($_SESSION['token'], $eta_id, count ($secteurs) ? $secteurs : NULL);
  return true;
}
...
function etablissement_add ($post) {
  ...
  $base->etablissement_secteurs_set ($_SESSION['token'], $new_grp_id, count ($secteurs) ? $secteurs : NULL);
  return true;  
}
