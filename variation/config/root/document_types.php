...
?>
<?php
$dtys_tous = $base->document_document_type_liste_par_sec_ids ($_SESSION['token'], 
							      (isset ($_POST['sec_id']) && $_POST['sec_id']) ? array ($_POST['sec_id']) : NULL, 
							      NULL, 
							      $base->order('dty_nom'));
...
<script type="text/javascript">
  $(document).ready (function () {
      ...
      $(".dty_intitule_individuel").click (on_dty_intitule_individuel_click);
...
function on_dty_intitule_individuel_click () {
  var ch = $(this).is(':checked');
  var dty = $(this).attr('id').substr (24);
  $.get ('/ajax/edit/document_document_type_set_nom_individuel.php', { prm_token: $("#token").val(), prm_dty_id: dty, prm_nom_individuel: ch ? 1 : 0}, function () {
      $("form").submit ();
    });
}
...
<?php liste_secteurs (arrval ($_POST, 'sec_id')); ?>
</select>
...
function affiche_types () {
  ...
  if (is_array ($dtys_tous)) {
	...
	foreach ($secs as $sec) {
      ...
      echo '<td align="center"><input type="checkbox" class="dty_intitule_individuel" id="dty_intitule_individuel_'.$dty['dty_id'].'"'.($dty['dty_nom_individuel'] ? ' checked' : '').'></input></td>';
      echo '<td>';
      if (is_array ($secs)) {
	foreach ($secs as $sec) {
