...
function on_ed_suppr_click () {
    if (confirm ('Supprimer la vue "<?= addslashes (arrval ($dos, 'dos_titre', '')) ?>" ?')) {
...
<?php } ?>
  <tr class="impair">
    <td>Nom</td>
     <td><input size="30" name="ed_nom" value="<?= arrval ($dos, 'dos_titre', '') ?>"></input><?= arrval ($erreur, 'ed_nom', '') ?></td>
  </tr>

  <tr class="pair">
    <td>Code</td>
     <td><input size="30" name="ed_code" value="<?= arrval ($dos, 'dos_code', '') ?>"></input><?= arrval ($erreur, 'ed_code', '') ?></td>
  </tr>
  ...
  <tr class="pair">
    <td>Type</td>
    <td><?php liste_types_cb (arrval ($dos, 'dty_id')) ?></td>
  </tr>
...
function liste_types_cb ($dty_id) {
  ...
  if (is_array ($types)) {
    foreach ($types as $type) {
      $selected = arrval ($type, 'dty_id') == $dty_id;
