...
$pagination->addColumn('Couleur', 'sec_color', null, null, 'color');
...
if (isset ($_GET['sec'])) {
  ...
  if (count ($erreur)) {
    ...
    $secteur['sec_couleur'] = $_POST['ed_color'];
  } else {    
...
<?php $pagination->displayData (array ('sec_code', 'sec_nom', 'sec_icone', 'sec_couleur')); ?>
...
if (isset ($_GET['sec'])) { 
  ...
  echo '<td>Libellé</td>';
  echo '<td>';
  echo '<input type="text" name="ed_nom" size="30" value="'.$secteur['sec_nom'].'" />'.arrval ($erreur, 'ed_nom', '');
  echo '</td>';
  echo '</tr>';
  ...
  echo '<td>Couleur</td>';
  echo '<td>';
  echo '<input type="text" name="ed_color" id="sme_color" value="'.$secteur['sec_couleur'].'" size="8" />'; 
  echo '<div class="colorbox">';
  foreach ($color_picker as $color){
      echo "<span class='color_picker' style='background-color:".$color."' onclick='$(\"#sme_color\").val(\"".$color."\")'></span>";
  }
  echo "</div>";
  echo '</td>';
  echo '</tr>';
...
function secteur_save($post) {
  ...
  $base->meta_secteur_save ($_SESSION['token'], $post['ed_code'], $post['ed_nom'], $post['ed_icone'], $post['ed_color']);
