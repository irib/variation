...
function on_ed_suppr_click () {
    if (confirm ('Supprimer la vue "<?= addslashes (arrval ($agr, 'agr_titre', '')) ?>" ?')) {
...
<?php } ?>
  <tr class="impair">
    <td>Nom</td>
    <td><input size="30" name="ed_nom" value="<?= arrval ($agr, 'agr_titre', '') ?>"></input><?= arrval ($erreur, 'ed_nom', '') ?></td>
  </tr>

  <tr class="pair">
    <td>Code</td>
    <td><input size="30" name="ed_code" value="<?= arrval ($agr, 'agr_code', '') ?>"></input><?= arrval ($erreur, 'ed_code', '') ?></td>
  </tr>
