...
function liste_portails () {
  ...
  foreach ($cats as $cat) {
    ...
    foreach ($pors as $por) {
      $selected = arrval ($_REQUEST, 'por') == $por['por_id'] ? ' selected' : '';
...
function liste_droits () {
 ...
 $droits_note = $base->droit_portail_get($_SESSION['token'], $portail['por_id'], 'suppression_notes');
 $droits_mail = $base->droit_portail_get($_SESSION['token'], $portail['por_id'], 'envoi_mail');

 echo '<span style="margin-right: 20px"><input type="checkbox" name="droit_suppr_note" id="droit_suppr_note"'.($droits_note ? ' checked': '').'></input><label for="droit_suppr_note">Suppression notes</label></span> ';
 echo '<span style="margin-right: 20px"><input type="checkbox" name="droit_mail" id="droit_mail"'.($droits_mail ? ' checked': '').'></input><label for="droit_mail">Envoi d\'email</label></span> ';
...
if (isset ($_POST['enreg'])) {
  ...
  $base->droit_portail_set($_SESSION['token'], $_REQUEST['por'], 'suppression_notes',  
					   isset ($_POST['droit_suppr_note']) && $_POST['droit_suppr_note'] == 'on');
  $base->droit_portail_set($_SESSION['token'], $_REQUEST['por'], 'envoi_mail',  
					   isset ($_POST['droit_mail']) && $_POST['droit_mail'] == 'on');
