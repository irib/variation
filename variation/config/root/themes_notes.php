...
$pagination->setFunction ('notes_theme_liste_details', array ($_SESSION['token'], arrval ($_GET, 'por')));
$pagination->setUrl ('/admin.php', array ('por' => arrval ($_GET, 'por')));
...
function on_ed_suppr_click () {
    if (confirm ('Supprimer la boîte thématique "<?= addslashes (arrval ($theme, 'the_nom', '')) ?>" ?')) {
...
function lien_edit_theme ($the_id) {
  return array ('/admin.php', array ('por' => arrval ($_GET, 'por'),
				     'the' => $the_id));
}
...
function liste_portails () {
  ...
  foreach ($cats as $cat) {
    ...
    foreach ($pors as $por) {
      $selected = $por['por_id'] == arrval ($_GET, 'por') ? ' selected' : '';
...
echo '<td>Nom</td>';
echo '<td>';
echo '<input type="text" name="ed_the_nom" size="30" value="'.arrval ($theme, 'the_nom', '').'"></input>';
echo arrval ($erreur, 'ed_the_nom', '');
echo '</td>';
echo '</tr>';
