...
if (isset ($_GET['evs'])) {
  ...
  if (count ($erreur)) {
    ...
    $evs['evs_id'] = isset ($evs_id) ? $evs_id : NULL;
    $evs['evs_titre'] = $_POST['ed_nom'];
    $evs['evs_code'] = $_POST['ed_code'];
    $evs['ety_id'] = $_POST['ed_ety'];
    ...
    if (isset ($_POST['ed_eca']) && count ($_POST['ed_eca'])) {
      foreach ($_POST['ed_eca'] as $s) {
	$evs_ecas[] = array ('eca_code' => $s);
      }
    }
...
function on_ed_suppr_click () {
    if (confirm ('Supprimer la vue "<?= addslashes (arrval ($evs, 'evs_titre', '')) ?>" ?')) {
...
<?php } ?>
  <tr class="impair">
    <td>Nom</td>
    <td><input size="30" name="ed_nom" value="<?= arrval ($evs, 'evs_titre', '') ?>"></input><?= arrval ($erreur, 'ed_nom', '') ?></td>
  </tr>

  <tr class="pair">
    <td>Code</td>
    <td><input size="30" name="ed_code" value="<?= arrval ($evs, 'evs_code', '') ?>"></input><?= arrval ($erreur, 'ed_code', '') ?></td>
  </tr>
  ...
  <tr class="impair">
    <td>Type</td>
    <td><?php liste_types_cb (arrval ($evs, 'ety_id')) ?></td>
  </tr>
...
function liste_types_cb ($ety_id) {
  global $base, $evs;
  $types = $base->events_event_type_list_par_evs ($_SESSION['token'], arrval ($evs, 'evs_id'));
