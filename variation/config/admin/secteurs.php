...
if (isset ($_POST['ed_enreg'])) {
  $base->secteur_infos_update ($_SESSION['token'], 
			       $_GET['sec'], 
			       arrval ($_POST, 'ed_editable') == 'on');
...
<table class="t1" width="370" style="margin-left: 480px">
  ...
  <tr class="impair"><td colspan="2">
    <input type="checkbox" id="ed_editable" name="ed_editable"<?= $ed_editable_checked ?>></input><label for="ed_editable">Les utilisateurs peuvent créer des partenaires et des groupes de cette thématique</label>
  </td></tr>
...
function liste_secteurs () {
  ...
  $impair = true;
  $secteurs = $base->etablissement_secteur_liste ($_SESSION['token'], NULL, $est_prise_en_charge);
