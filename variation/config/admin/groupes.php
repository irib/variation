...
$pagination->setFunction ('groupe_liste_details', array ($_SESSION['token'], 
							 arrval ($_GET, 'eta_id'), 
							 arrval ($_GET, 'sec_id_role'), 
							 arrval ($_GET, 'sec_id_besoin'), 
							 true));
$pagination->setUrl ('/admin.php', array ('eta_id' => arrval ($_GET, 'eta_id'),
					  'sec_id_role' => arrval ($_GET, 'sec_id_role'),
...
function on_ed_suppr_click () {
    if (confirm ('Supprimer le groupe "<?= addslashes (arrval ($groupe, 'grp_nom', '')) ?>" ?')) {
...
<option value="">(tous établissements)</option>
  <?php liste_etablissements (arrval ($_GET, 'eta_id')); ?>
</select>
...
if (isset ($_GET['grp'])) { 
  ...
  liste_etablissements (arrval ($groupe, 'eta_id'));
  echo '</select>'.arrval ($erreur, 'ed_etab', '');
} 
echo '</td>';
echo '</tr>';
...
if ((isset ($_POST['ed_etab']) && $_POST['ed_etab']) || isset ($_GET['grp'])) {
  ...
  echo '</input>'.arrval ($erreur, 'ed_intitule', '').'</td>';
  echo '</tr>';

  echo '<tr class="impair">';
  echo '<td>Capacité</td>';
  echo '<td><input style="text-align: right" size="5" name="ed_capacite" value="'.$groupe['grp_capacite'].'">';
  echo '</input>'.arrval ($erreur, 'ed_capacite', '').'</td>';
  echo '</tr>';
...
function lien_edit_groupe ($grp_id) {
  return array ('/admin.php', array ('eta_id' => arrval ($_GET, 'eta_id'),
				     'sec_id_role' => arrval ($_GET, 'sec_id_role'),
				     'sec_id_besoin' => arrval ($_GET, 'sec_id_besoin'),
				     'grp' => $grp_id));
}
...
function groupe_save ($post) {
  ...
  // Capacité
  $base->groupe_set_capacite ($_SESSION['token'], $grp_id, $post['ed_capacite']);
