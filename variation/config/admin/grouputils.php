...
function on_ed_suppr_click () {
    if (confirm ('Supprimer le groupe d\'utilisateurs "<?= addslashes (arrval ($grouputil, 'gut_nom')) ?>" ?')) {
...
echo '<td>Intitulé</td>';
echo '<td>';
echo '<input type="text" size="30" name="ed_intitule" value="'.arrval ($grouputil, 'gut_nom', '').'"></input>'.arrval ($erreur, 'ed_intitule', '');
echo '</td>';
echo '</tr>';
