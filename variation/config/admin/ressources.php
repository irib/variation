...
$pagination->setFunction ('ressource_liste_details', array ($_SESSION['token'], 
							    arrval ($_GET, 'sec_id')));
...
function on_ed_suppr_click () {
    if (confirm ('Supprimer la ressource "<?= addslashes (arrval ($ressource, 'res_nom', '')) ?>" ?')) {
...
?>
  <tr class="pair">
    <td>Ressource</td>
    <td><input type="text" value="<?= arrval ($ressource, 'res_nom', '') ?>" name="ed_res_nom"></input>
      <?php echo arrval ($erreur, 'ed_res_nom', ''); ?>
    </td>
  </tr>
  <tr class="impair">
    <td>Thématiques</td>
