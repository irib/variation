...
if (isset ($_POST['ed_enreg'])) {
  ...
  exit;
} else if (arrval ($_POST, 'mdp_generer')) {
...
function utilisateur_add ($post) {
  ...
  $new_uti_id = $base->utilisateur_add ($_SESSION['token'], $post['ed_login'], $post['ed_per_id'], 
					$post['ed_uti_config'], $post['ed_uti_root'], $post['ed_email']);
...
function on_ed_suppr_click () {
    if (confirm ('Supprimer l\'utilisateur "<?= addslashes (arrval ($util, 'uti_login', '')) ?>" ?')) {
...
echo '<td>Login</td>';
echo '<td>';
echo '<input type="text" size="30" name="ed_login" value="'.arrval ($util, 'uti_login', '').'"></input>'.arrval ($erreur, 'ed_login', '');
echo '</td>';
echo '</tr>';
...
echo '<td>Email</td>';
echo '<td>';
echo '<input type="text" size="30" name="ed_email" value="'.arrval ($util, 'uti_email', '').'"></input>'.arrval ($erreur, 'ed_email', '');
echo '</td>';
echo '</tr>';
...
echo '<td>';
echo '<input type="checkbox" name="ed_uti_config" id="ed_uti_config"'.(arrval ($util, 'uti_config') ? ' checked' : '').'><label for="ed_uti_config">Établissement</label><br/>';
echo '<input type="checkbox" name="ed_uti_root" id="ed_uti_root"'.(arrval ($util, 'uti_root') ? ' checked' : '').'><label for="ed_uti_root">Système</label>';
echo '</td>';
echo '</tr>';
