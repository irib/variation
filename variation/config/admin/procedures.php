...
function procedure_save ($post) {
  ...
  if (count ($erreur))
    return false;
  $base->procedure_procedure_update ($_SESSION['token'], 
				     arrval ($_GET, 'pro'), 
				     $post['ed_pro_titre'], 
				     $post['ed_pro_contenu']);
}
...
function on_paf_add_click () {
  $("#dlg").ficheSelect ({
	...
	prm_token: $("#token").val(),
	  prm_pro_id: <?php echo arrval ($_GET, 'pro', '') ?>,	  
	  prm_tsm_id: tsm_id,
	  prm_sme_id: sme_id
	  }, function (data) {
	  location.reload ();
...
function on_ed_suppr_click () {
    if (confirm ('Supprimer la procédure "<?= addslashes (arrval ($procedure, 'pro_titre', '')) ?>" ?')) {
...
function liste_fiches ($procedure_affectations) {
  global $base;
  if (is_array ($procedure_affectations)) {
...
echo '<td>Titre</td>';
echo '<td>';
echo '<input type="text" name="ed_pro_titre" size="30" value="'.arrval ($procedure, 'pro_titre', '').'"></input>';
echo arrval ($erreur, 'ed_pro_titre', '');
echo '</td>';
echo '</tr>';
...
echo '<tr class="impair">';
echo '<td colspan="2">';
echo '<textarea cols="73" rows="20" name="ed_pro_contenu">'.arrval ($procedure, 'pro_contenu', '').'</textarea>';
echo '</td>';
echo '</tr>';
