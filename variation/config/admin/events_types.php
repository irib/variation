...
$etys_tous = $base->events_event_type_list ($_SESSION['token'], 
					    arrval ($_POST, 'eca_id'), 
					    (isset ($_POST['sec_id']) && $_POST['sec_id']) ? array ($_POST['sec_id']) : NULL, 
					    NULL);

if (arrval ($_POST, 'enreg')) {
...
<?php liste_categories (arrval ($_POST, 'eca_id')); ?>
</select>
...
function liste_secteurs ($sec_id) {
  global $base;
  $secs = $base->etablissement_secteur_liste ($_SESSION['token'], NULL, NULL);
  foreach ($secs as $sec) {
