...
$metier_secteurs = $base->metier_secteur_liste ($_SESSION['token'], arrval ($_GET, 'met'));
...
$pagination->setFunction ('metier_liste_details', array ($_SESSION['token'], arrval ($_GET, 'ent_id'), arrval ($_GET, 'sec_id')));
$pagination->setUrl ('/admin.php', array ('ent_id' => arrval ($_GET, 'ent_id'),
					  'sec_id' => arrval ($_GET, 'sec_id')
					  ));
...
function on_ed_suppr_click () {
    if (confirm ('Supprimer le métier "<?= addslashes (arrval ($metier, 'met_nom', '')) ?>" ?')) {
...
<?php liste_entites (arrval ($_GET, 'ent_id')); ?>
</select>
<select name="sec_id">
...
echo '<td>Métier</td>';
echo '<td>';
echo '<input type="text" size="30" name="ed_intitule" value="'.arrval ($metier, 'met_nom', '').'"></input>'.arrval ($erreur, 'ed_intitule', '');
echo '</td>';
echo '</tr>';
...
function lien_edit_metier ($met_id) {
  return array ('/admin.php', array ('ent_id' => arrval ($_GET, 'ent_id'),
				     'sec_id' => arrval ($_GET, 'sec_id'),
				     'met' => $met_id));
}
