 ...
 */
class AccueilCalPrincipal extends DAVACL\PrincipalBackend\AbstractBackend implements DAVACL\PrincipalBackend\CreatePrincipalSupport {
...
;
	...
	$utils = $this->base->utilisateur_liste_details_configuration($this->auth->token);
	$principals = array();
	foreach ($utils as $util) {
	  $principals[] = [ 'uri' => 'principals/' . $util['uti_login'] ,
			    '{DAV:}displayname' => $util['uti_prenom'].' '.$util['uti_nom']
			    ];
	}
        return $principals;
	...
	$login = substr($path, strlen('principals/'));
	$util = $this->base->utilisateur_get_par_login($login);
	//	file_put_contents('/tmp/digest.txt', print_r($util, true));
	$principal = [
		      'id' => $util['uti_id'],
		      'uri' => $path,
		      '{DAV:}displayname' => $this->base->utilisateur_prenon_nom($this->auth->token, $util['uti_id']),
		      '{http://sabredav.org/ns}email-address' => $util['uti_email']		      
		      ];
        return $principal;
