...
class Digest extends AbstractAuth {
   ...
   public function getValidationString() {
        ...
        $validResponse = "{$this->digestParts['nonce']}:{$this->digestParts['nc']}:{$this->digestParts['cnonce']}:{$this->digestParts['qop']}:{$A2}";

	return array('response' => $this->digestParts['response'], "part" => $validResponse);
    }
    /**
