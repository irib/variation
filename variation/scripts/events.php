    ...
    <fieldset class="events">
        ...
        <legend>Recherche</legend> 
            ...
            <div class="choix_vue" id="periode">
                <h2>Période</h2>
                <h3>Du </h3>
                <input name="events_date_debut" type="text" size="9" class="datepicker" value="<?php echo arrval ($_GET, 'events_date_debut', '') ?>"/>
                <br />
                <h3>Au </h3>
                <input name="events_date_fin" type="text" size="9" class="datepicker" value="<?php echo arrval ($_GET, 'events_date_fin', '') ?>"/>
            </div>
                <?php
                }
                ?>
...
<?php
	...
	$liste="";
	$bilan="";
	switch ($_GET['events_vue']){
		case 'liste'; $liste="actived"; break;
		case 'bilan'; $bilan="actived"; break;
		default; $agenda="actived"; break;
	}         

    if ($_GET['events_vue']=='liste' or $_GET['events_vue']=='bilan') {
      ?></form>
      <form action="">
        <fieldset style="float:left;"><legend>Recherche</legend>
      
          <input type="hidden" name="ent" value="<?= $_GET['ent'] ?>" />
          <input type="hidden" name="per" value="<?= $_GET['per'] ?>" />
          <input type="hidden" name="sme" value="<?= $_GET['sme'] ?>" />
          <input type="hidden" name="events_vue" value="<?= $_GET['events_vue'] ?>" />
          ...
          <?php if ($_GET['events_vue']=='liste'){ ?>
           <div class="choix_vue">
                <h2>Option</h2>
                <?php $affiche_echeances = (isset ($_GET['affiche_echeances']) && $_GET['affiche_echeances'] == 'on') ? 'checked' :  ''; ?>
                <input type="checkbox" id="affiche_echeances" name="affiche_echeances" <?php echo $affiche_echeances ?> />
                <label for="affiche_echeances" style="font-size: 9pt">Afficher début/fin</label>        
            </div> 
          <?php } ?>
    ...
    <fieldset class="usager_agenda_filtre">
        <legend>Type de vue</legend>
        <div class="choix_vue">
            <div id="events_vues"> 
                <p>
				<a class="<?= $agenda ?>" href="edit.php?ent=<?= $_GET['ent'] ?>&per=<?= $_GET['per'] ?>&sme=<?= $_GET['sme'] ?>&events_vue=agenda">Agenda</a>
				<a class="<?= $liste  ?>" href="edit.php?ent=<?= $_GET['ent'] ?>&per=<?= $_GET['per'] ?>&sme=<?= $_GET['sme'] ?>&events_vue=liste">Liste</a>
				<a class="<?= $bilan  ?>" href="edit.php?ent=<?= $_GET['ent'] ?>&per=<?= $_GET['per'] ?>&sme=<?= $_GET['sme'] ?>&events_vue=bilan">Bilan</a>
				</p>
            </div>
        </div>
    </fieldset>
...
if (isset ($_GET['events_vue']) && $_GET['events_vue'] == 'liste') {
  ...
  $endDate = arrval ($_GET, 'events_date_fin');
  $grp_id = arrval ($_GET, 'rech_grp_id');
  $code = $evs['evs_code'];
  $per_ids = arrval ($_GET, 'rech_per_id');
  $affiche_echeances = arrval ($_GET, 'affiche_echeances') == 'on';
  ...
  if (isset ($events)) {
	...
	foreach ($participants as $p) {
	  ...
	  if($p['per_id']==$_GET['per']){
		  $per_in_array=True;
	  }
	}	
	if($per_in_array or empty($_GET['per'])){
	...
	if(($_GET['per']==$event['per_id']) or (empty($_GET['per']))){
...
if (isset ($_GET['events_vue']) && $_GET['events_vue'] == 'bilan') {
  ...
  $eves = $base->events_event_bilan ($_SESSION['token'], 
				     $evs['evs_id'], 
				     isset ($per_id) ? $per_id : NULL, 
				     $startDate, $endDate, 
				     $grp_id, 
				     $per_ids);
...
function liste_groupes () {
  ...
  foreach ($grps_prise_en_charge as $grp_pec) {
    if ($grp_pec['eta_id'] != $_SESSION['eta_id']) continue;
