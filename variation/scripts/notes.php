...
function on_envoi_note_click () {
  ...
  if (!deststheme.length) {
    alert ("Vous devez saisir au moins une boîte thématique.");
    return;
  }  
  if (!dests.length)
    dests = '';
  ...
  $.post ('/ajax/edit/notes_note_ajoute.php',  { 
	...
	prm_groupes: groupes
      ...
      $.post ('/scripts/notes_envoi_mail.php', {
        prm_token: $("#token").val(),
              prm_date_evenement: date_evenement,
              prm_objet: objet, 
              prm_texte: texte,
...
function affiche_mesnotes () {
  ...
  $usagers = $base->utilisateur_usagers_liste ($_SESSION['token'], 
					       arrval ($_SESSION, 'grp'), 
					       true, 
					       $base->order('libelle'));  
  ...
  echo '<br>Texte de la note :<br><textarea id="not_texte" name="not_texte" rows="6" style="width: 100%; resize: none; box-sizing: border-box; "></textarea>';
  ...
  if ($per_id) {
    $nnotes = $base->notes_note_usager_liste ($_SESSION['token'], $per_id, $sousmenu['sme_type_id'], null, null, $base->count());
    $notes = $base->notes_note_usager_liste ($_SESSION['token'], 
					     $per_id, 
					     $sousmenu['sme_type_id'], null, null,
					     $base->order (arrval ($_SESSION, 'tripar') ? $_SESSION['tripar'] : 'not_date_evenement', 'DESC'), 
					     $base->limit (NPAGES, ($p-1)*NPAGES));
  } else {
    $grps = $base->utilisateur_groupe_liste ($_SESSION['token']);
    $grp_choisi = arrval ($_SESSION, 'grp');
    ...
    $notes = $base->notes_note_mesnotes ($_SESSION['token'], $grp_choisi, $tsm['tsm_type_id'], 
					 $base->order (arrval ($_SESSION, 'tripar') ? $_SESSION['tripar'] : 'not_date_evenement', 'DESC'), $base->limit (NPAGES, ($p-1)*NPAGES));
  }
  ...
  $impair = true;
  if (is_array ($notes)) {
	$droits_suppr_note = $base->droit_portail_get($_SESSION['token'], $_SESSION['portail'], 'suppression_notes');
    foreach ($notes as $note) {
      ...
      $uti_auteur = $base->utilisateur_get($_SESSION['token'], $note['uti_id_auteur']);
      echo '<tr class="'.($impair ? 'impair' : 'pair').'">';
      echo '<td>'.substr ($note['not_date_evenement'], 0, 16).'<br><span class="lienpersonne" id="lienpersonne_personnel_'.$uti_auteur['per_id'].'"> '.$auteur.'</span></td>';
      echo '<td>';
      foreach ($usagers as $usager) {
	...
	foreach ($destinataires as $destinataire) {
      ...
      echo '</td>';
      echo '<td>';
      if($droits_suppr_note)echo '<button type="button" id="note_del_'.$note['not_id'].'" class="note_del">Supprimer</button>';
