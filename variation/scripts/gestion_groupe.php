...
$etab_secteurs = $base->etablissement_secteur_liste ($_SESSION['token'], $_SESSION['eta_id'], NULL);
...
//  if (isset ($groupe['eta_id'])) 
  liste_etablissements_editables (arrval ($groupe, 'eta_id'));
  echo '</select>'.(isset ($erreur['ed_etab']) ? $erreur['ed_etab'] : '').'<br><a id="nouveletab" href="#">créer un nouveau partenaire</a>';
  }
...
if ((isset ($_POST['ed_etab']) && $_POST['ed_etab']) || isset ($_GET['grp'])) {
  ...
  echo '<td>';
  if (isset ($_GET['grp'])) {
    liste_secteurs_cb ($groupe_secteurs, null, true);
   } else {
    liste_secteurs_cb ($groupe_secteurs, $secteur1['sec_id'], true);
  }
  ...
  echo '<td>';
  if (isset ($_GET['grp'])) {
    liste_secteurs_cb ($groupe_secteurs, null, false);
   } else {
    liste_secteurs_cb ($groupe_secteurs, $secteur1['sec_id'], false);
  }
...
function liste_secteurs_cb ($groupe_secteurs, $sec_id_checked = null, $est_prise_en_charge = true) {
  ...
  foreach ($secteurs as $secteur) {
    ...
    if (!$affiche)
      continue;
    $disabled = !secteur_editable ($secteur);
    $checked = false;
...
function secteur_editable ($secteur) {
  global $etablissement, $base, $sec_id;
  if (!$etablissement['cat_id']) {
    // partenaire
    if ($secteur['sec_id'] == $sec_id)
      return true;
    $si = $base->secteur_infos_get ($_SESSION['token'], $secteur['sec_id']);
    return $si['sec_editable'];
  } else {
    // étab
    if ($secteur['sec_id'] == $sec_id)
      return true;
    return $base->etablissement_secteur_edit_get ($_SESSION['token'], $etablissement['eta_id'], $secteur['sec_code']);
  }
}
?>
