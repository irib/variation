...
$ok_mail = false;
if (isset($_POST['mail'])){
    $base->utilisateur_update_profil($_SESSION['token'], $_SESSION['uti_id'], $_POST['mail']);
    $ok_mail = "Votre adresse email est enregistré";
}
...
<style type="text/css">
  ...
  <?php if (isset ($mode_demo) && $mode_demo === true) { ?>
...
<form method="POST" action="/?profil">
      <fieldset><legend> Modifier mon email </legend>
      <label for="mail">Adresse email: </label>
      <input type="email" name="mail" id="mail" value="<?php $uti = $base->utilisateur_get($_SESSION['token'], $_SESSION['uti_id']); echo $uti['uti_email']; ?>" />
      <div class='ok'><?php echo $ok_mail; ?></div>
      <div class='buttons'>
     <button type='submit' id="btn_enreg_mail">Modifier</button>
     </div>
     </fieldset>
</form>
<?php } ?>
