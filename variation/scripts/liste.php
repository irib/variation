...
<div class='scrollablex'><table class="tableliste">
<?php 
  if ($lis['lis_inverse']) {
    affiche_inverse ();
  } else {
...
function on_ajout_personne_click () {
  if(<?= $portail['por_droit_ajout_'.$entite[$lis['ent_id']]['ent_code']] ? 'true': 'false' ?>){
  $("#dlg").personneAjout({
      "por_id": $("#por_id").val(),
	"eta_id": $("#eta_id").val(),
  ...
  $("#dlg").position();
	  ...
	  alert('Vous ne disposez pas des droits nécessaires');
  }
}
...
function sup_col_gauche () {
  ...
  foreach ($infos as $info) {
	...
	foreach ($cycle_statuts as $k => $lib) {
	  $selected = arrval ($_GET, 'rech-'.$info['inf_code'].'-sta') == $k ? ' selected' : '';
	  $ret .= '<option'.$selected.' value="'.$k.'">'.$lib.'</option>';
	}
	$ret .= '</select><br/>';
	$ret .= 'Du <input name="rech-'.$info['inf_code'].'-du'.'" class="datepicker" type="text" size="8" value="'.arrval ($_GET, 'rech-'.$info['inf_code'].'-du', '').'"></input><br/>';
	$ret .= 'Au <input name="rech-'.$info['inf_code'].'-au'.'" class="datepicker" type="text" size="8" value="'.arrval ($_GET, 'rech-'.$info['inf_code'].'-au', '').'"></input><br/>';	
