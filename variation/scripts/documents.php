...
$col_order_available = array('doc_statut','dty_id','doc_date_obtention','doc_date_realisation','doc_date_validite');
if(!isset($_GET['col'])){
	$col_order = "doc_titre";
}
elseif(in_array($_GET['col'],$col_order_available)){
	$col_order = $_GET['col'];
}else{
	$col_order="doc_titre";
}
...
if (isset ($per_id)) {
  ...
  $droit_modif = $tsm['tsm_droit_modif'];
    $count1 = $base->document_document_liste ($_SESSION['token'], $dos_id, NULL, 
					      arrval ($_GET, 'documents_date_debut'),
					      arrval ($_GET, 'documents_date_fin'),
...
<input type="hidden" id="tsm_id" value="<?= isset ($tsm) ? $tsm['tsm_id'] : '' ?>"></input>
<input type="hidden" id="sme_id" value="<?= arrval ($sousmenu, 'sme_id', '') ?>"></input>
<?php
  if (isset ($tsm) && $tsm['tsm_id'] != '') {
?>
...
<table class="tabledocs" width="100%">
    <tr>
		...
		function genere_url($col,$per_id){
			if(!isset($per_id)){
				$urlBase="/?";
			}
			else {
			...
			$sort="ASC";
			if($colValue==$col){
				if($sortValue=="ASC")$sort="DESC";
			}
			$url .= "sort=".$sort."&col=".$col;
			return $url;
		}
		function show_img_nav($col){
			if($col==$_GET['col']){
				if($_GET['sort']=='DESC'){return '<img src="/Images/navig/Desc.gif">';}
				else{return '<img src="/Images/navig/Asc.gif">';}
			}
			else return "";
		}
		?>
        <th><a href="<?php	echo genere_url('doc_titre',$per_id);	?>">	Titre 	<?= show_img_nav('doc_titre') ?></a></th>
        <th><a href="<?php	echo genere_url('doc_statut',$per_id);	?>">	Statut	<?= show_img_nav('doc_statut') ?></a></th>
        <th>Usagers</th>
        <th><a href="<?php	echo genere_url('dty_id',$per_id);		?>">	Type	<?= show_img_nav('dty_id') ?></a></th>
        <th><a href="<?php	echo genere_url('doc_date_obtention',$per_id);	?>">	Limite<br/>obtention	<?= show_img_nav('doc_date_obtention') ?></a></th>
        <th><a href="<?php	echo genere_url('doc_date_realisation',$per_id);?>">	Réalisation				<?= show_img_nav('doc_date_realisation') ?></a></th>
        <th><a href="<?php	echo genere_url('doc_date_validite',$per_id);	?>">	Validité				<?= show_img_nav('doc_date_validite') ?></a></th>
        <th>Responsable</th>
        <th>&nbsp;</th>
    </tr>
  <?= affiche_docs (); ?>
