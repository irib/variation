...
$filename = $arrangement.'/arrangement/intitules.csv';
...
while ( ($tmp = fgetcsv ($f, 0, "\t")) !== FALSE) {
  ...
  $ter_code = $ligne['code_intitule'];
  $defaut = $ligne['defaut'];
  $base->localise_par_code_secteur_set($token, $ter_code, null, $defaut);
  foreach ($codes_secteurs as $sec_code) {
    if ($ligne[$sec_code]) {
      $base->localise_par_code_secteur_set($token, $ter_code, $sec_code, $ligne[$sec_code]);      
    }
  }
}
