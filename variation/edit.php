...
function remplace_tags ($matches) {
  ...
  case 'famille': 
	...
	foreach ($membres as $membre) {
      ...
      $chaine = '<span id="lienpersonne_famille_'.$membre['per_id_parent'].'" class="lienpersonne">'.$nom.'</span>';
      if(!in_array($chaine, $ms)){
          $ms[] = $chaine;
      }
	}
      }
...
function affiche_entete () {
  ...
  $btn_print="";
  if($ent_code == 'usager'){
	  $btn_print = '<a id="print_dossier_button" class="smallbutton">Imprimer le dossier</a>';
  }
  echo '<h1>'.$nom.' '.$prenom.' ('.$entite['ent_libelle'].')'.$btn_print.'</h1>';  
  ...
  echo "  $('#print_dossier_button').click (on_print_dossier_button_click);";
  echo "});</script>";
  echo '</div>';
  echo '</div>';
...
function affiche_info_groupe ($info) {
  ...
  if (count ($groupes)) {
    ...
    foreach ($groupes as $groupe) {
      ...
      $etab_info = $base->etablissement_get($_SESSION['token'], $groupe['eta_id']);
      if($etab_info['adr_id']){
          $etab_adr = $base->adresse_get($_SESSION['token'], $etab_info['adr_id']);
          $qtip = "<strong>Adresse: </strong>".$etab_adr['adr_adresse'].", ".$etab_adr['adr_cp']." ".$etab_adr['adr_ville']."<br /><strong>Téléphone: </strong>".$etab_adr['adr_tel']."<br /><strong>Fax: </strong>".$etab_adr['adr_fax']."<br /><strong>Email: </strong>".$etab_adr['adr_email'];
      }
      else{
          $qtip = "Aucune information pour cet etablissement.";
      }
      echo "<span style=\"cursor: pointer; border-bottom: 1px dotted #888\" class='qtipable' title=\"".$qtip."\">".$groupe['eta_nom'].'</span> &rarr; '.$groupe['grp_nom'];
...
function affiche_info_contact ($info) {
  ...
  if ($ent_code == 'usager') {
	...
	echo '<span id="infocontact_ajout_'.$info['inf_code'].'" class="infocontact_ajout">Ajouter</span>';
      ...
      $classedel = $droit_modif ? 'infocontact1del' : 'infocontact1del_ro';
      $valeur = $base->personne_info_integer_get_details ($_SESSION['token'], $per_id, $info['inf_code']);
      $nomprenom = $base->personne_get_libelle ($_SESSION['token'], $valeur['pii_valeur']);
      echo '<span class="'.$classe.'" id="infocontact1_'.$info['inf_code'].'">';
      if ($valeur['pii_id'])
	echo '<span class="'.$classedel.'" id="infocontact1del_'.$valeur['pii_id'].'">';
      echo '<nobr><span id="lienpersonne_'.$info['inf__contact_filtre'].'_'.$valeur['pii_valeur'].'" class="lienpersonne">'.$nomprenom.'</span></nobr>';
      if ($valeur['pii_id'])
	echo '</span>';
      echo '</span>';
    }
...
function affiche_info_etablissement ($info) {
  ...
  if ($multiple) {
    ...
    $classedel = $droit_modif ? 'infoetab1del' : 'infoetab1del_ro';
    $valeur = $base->personne_info_integer_get_details ($_SESSION['token'], $per_id, $info['inf_code']);
    $etab = $base->etablissement_get ($_SESSION['token'], $valeur['pii_valeur']);
    echo '<span class="'.$classe.'" id="infoetab1_'.$info['inf_code'].'">';
    if ($valeur['pii_id'])
      echo '<span class="'.$classedel.'" id="infoetab1del_'.$valeur['pii_id'].'">';
    echo '<nobr>'.$etab['eta_nom'].'</nobr>';
    echo '</span>';
  }
  return;
}
...
function affiche_info_affectation ($info) {
  ...
  if ($multiple) {
    ...
    foreach ($valeurs as $valeur) {
      ...
      if($etab['adr_id']){
          $etab_adr = $base->adresse_get($_SESSION['token'], $etab['adr_id']);
          $qtip = "<strong>Adresse: </strong>".$etab_adr['adr_adresse'].", ".$etab_adr['adr_cp']." ".$etab_adr['adr_ville']."<br /><strong>Téléphone: </strong>".$etab_adr['adr_tel']."<br /><strong>Fax: </strong>".$etab_adr['adr_fax']."<br /><strong>Email: </strong>".$etab_adr['adr_email'];
      }
      else {
          $qtip = "Aucune information pour cet etablissement.";
      }
      echo '<span id="pij_'.$valeur['pij_id'].'" class="'.$classe.' qtipable" title="'.$qtip.'"><nobr>'.$etab['eta_nom'];
      if ($valeur['valeur2']) {
...
function affiche_info_famille ($info) {
  ...
  if ($multiple) {
	...
	foreach ($valeurs as $valeur) {
	  ...
	  $autorite = $valeur['pif_autorite_parentale'] == 1 ? 'Autorité parentale' : ($valeur['pif_autorite_parentale'] == 2 ? 'Tutelle' :($valeur['pif_autorite_parentale'] == 3 ? 'Curatelle' : 'Aucune autorité'));
	...
	foreach ($valeurs as $valeur) {
	  $lien = $base->meta_lien_familial_get($_SESSION['token'], $valeur['lfa_id']);
	  $autorite = $valeur['pif_autorite_parentale'] ? 'Autorité parentale' : ($valeur['pif_autorite_parentale'] == 2 ? 'Tutelle' : 'Curatelle');
    ...
    echo 'Famille non multiple Non implémenté';
  }
}
