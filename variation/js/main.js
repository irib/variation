...
$(document).ready (function () {
	...
	$(".infoetab1del").click (on_infoetab1_del);
	...
	$(".infocontact1del").click (on_infocontact1_del);
...
function on_membrefam_ajout () {
	...
	var droit_famille = $('#droit_famille').val()=='1';
	...
	return: function (id, type, nom, lfa, lfanom, autorite_parentale, droits, periodicite) {
		$.getJSON ('/ajax/edit/personne_info_lien_familial_set.php', {
			...
			var autorite = "Aucune autorité";
			if(autorite_parentale == 1) autorite = "Autorité parentale";
			if(autorite_parentale == 2) autorite = "Tutelle";
			if(autorite_parentale == 3) autorite = "Curatelle";
			var qtip = '<u>' + lfanom + ' : ' + nom + '</u><br>' + autorite + "<br>" + droits + '<br>' + periodicite;
			$("#"+inf_code+"_list").append('<span title="'+qtip+'" id="lfa_'+data+'" class="membrefam cliquable qtipable"><nobr>'+lfanom+' : '+nom+'</nobr></span> ');
			$("#lfa_"+data).qtip({
			    position : {
				corner : {
				    target : 'topMiddle',
...
function on_infoetab1_del () {
    var ok = confirm ("Voulez-vous supprimer ce lien ?");
    if (!ok)
	return false;
    var pii_id = $(this).attr('id').substr (13);
    var toremove = $(this);
...
function on_infocontact1_click () {
    ...
    var per_id = $("#per_id").val();
    var clicked = $(this);
	var droit_contact = $('#droit_contact').val()=='1';
    $.getJSON ('/ajax/meta_info_get_par_code.php', { prm_token: $("#token").val(), prm_inf_code: inf_code, output: 'json2' }, function (res) {
	    $("#editdlg").participantSelect ({
...
function on_infocontact1_del () {
    var ok = confirm ("Voulez-vous supprimer ce lien ?");
    if (!ok)
	return false;
    var pii_id = $(this).attr('id').substr (16);
    var toremove = $(this);
    $.getJSON ('/ajax/edit/personne_info_integer_delete.php', { prm_token: $("#token").val(), prm_pii_id: pii_id }, function () {
	toremove.remove();
	});
