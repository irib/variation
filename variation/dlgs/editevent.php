...
$(document).ready (function () {
    ...
    $("#ajout_participant_myself").click (on_ajout_myself_click);
    if($('#participant_'+$('#per_id_myself').val()).html()!=null){$('#ajout_participant_myself').hide()}
    $('#eve_debut').change(on_date_change);
    ...
    $("#case_repeat").click (on_case_repeat_click);
    ...
    $("#repeat_type").change (on_repeat_type_change);
...
function on_date_change(){
	var chaine_date=$('#eve_debut').val();
	date = date_fr_to_date(chaine_date);
	semaine = ['dimanche','lundi','mardi','mercredi','jeudi','vendredi','samedi'];
	ordre = ['premier','second','troisieme','quatrième','cinquième'];
	$('#repeat_month_date').text(date.getDate());
	$('#repeat_month_day').text(semaine[date.getDay()]);
	dif_ordre_number=date.getDate();
	ordre_number=0;
	while(dif_ordre_number>7){
		dif_ordre_number-=7
		ordre_number+=1;
	}
	$('#repeat_month_day_number').text(ordre[ordre_number]);
	}
function on_participant_click (e) {
    if (confirm ('Vous allez supprimer ce participant pour cet événement. Continuer ?')) {
        id = $(this).attr('id');
        if("participant_"+$('#per_id_myself').val() == id){
            $('#ajout_participant_myself').show();
        }
        $(this).remove ();
    }
}
function on_ajout_myself_click() {
    $('#ajout_participant_myself').hide();
    $('#participants-personnel').append( '<span id="participant_'+$('#per_id_myself').val()+'" class="participant"><nobr><span>Moi-même</span></nobr></span>');
    $("#participants-personnel > .participant").unbind('click').click (on_participant_click);
    $("#participants-personnel > .participant > nobr > span").unbind('click))').click(function() {return false;});
}
function on_ajout_participant_click () {
  ...
  droit_ajout=false;
  <?php
    $droits_ajout = $base->droit_ajout_entite_portail_liste_par_portail($_SESSION['token'],$_SESSION['portail']);
    foreach($droits_ajout as $droit){
      if($droit['daj_droit'])echo "if (the_type=='".$droit['ent_code']."')droit_ajout=true;";
    }
  ?>
...
function on_case_repeat_click() {
	if($('#repeat').is(':checked')){
		$('#repeat_form').slideDown(100);
		on_date_change();
	}else{
		$('#repeat_form').slideUp(100);
	}
}
...
function on_repeat_type_change() {
	var value=$(this).val();
	if(value=='month'){
		$('#repeat_all_day').hide();
		$('#repeat_all_month').show();
	}if(value=='day'){
		$('#repeat_all_day').show();
		$('#repeat_all_month').hide();
		$('#repeat_day_delay').focus();
	}
}
...
function on_event_enregistrer_click () {
	...
	repetitions = 0;
	if($('#repeat').is(':checked')){
		repetitions = $('#number_of_repeat').val();
		if(!(is_int(repetitions))){
			alert("La valeur saisie du nombre de répétion n'est pas correcte.");
			return;
		}
		if(repetitions==0){
			alert("Vous ne pouvez pas avoir 0 répétition");
			return;
		}
		if($('#repeat_type').val()=='day'){
			recurrent=1;
			jour_intervalle=$('#repeat_day_delay').val();
			if(!(is_int(jour_intervalle))){
				alert("La valeur saisie du nombre de jour d'intervalle n'est pas correcte.");
				return;
			}
			if(jour_intervalle==0){
				alert("Vous ne pouvez pas avoir 0 jours d'intervalle");
				return;
			}
		}else{
			if($('#number_radio').is(':checked')){
				recurrent=2;
			}
			else{
				recurrent=3;
	...
	if(!check_periode(tmp_debut, tmp_fin)){
    ...
    $.post ('/ajax/edit/events_event_save_all.php', {
		...
		prm_res_ids: res_ids,
		prm_sec_ids: sec_ids,
		prm_recurrent:recurrent,
		prm_jour_intervalle: jour_intervalle,
            prm_repetitions: repetitions,
            prm_lieu: $('#eve_lieu').val()
    }, function (data) {
	$("#dlg").dialog ('destroy');
	if(typeof($("#calendar").fullCalendar) === "function"){
		$("#calendar").fullCalendar ('refetchEvents');
	}else{
		window.location.reload(); //Si en mode liste ou bilan, alors on recharge la page
	}
    });        
}
...
<input type="hidden" id="per_id_usager" value="<?= $_GET['per_id'] ?>"></input>
<input type="hidden" id="per_id_myself" value="<?php
    ...
    <table class="tabledlg">
		...
		<tr>
		  ...
		  <tr>
			...
			<td colspan="4"><?php if(empty($eve_id)) { ?>
				<span id="case_repeat"><input type="checkbox" id="repeat" /> <label for="repeat">&Eacute;vènement récurrent</label></span>
			
				<div id="repeat_form" style='display:none;'>
					Répétion
						<select id='repeat_type'>
							<option value='day'>Par jour(s)</option>
							<option value='month'>Par mois</option>
						</select>
						
					<span id='repeat_all_month' style='display:none'>

						
						<div id='repeat_all_month_by_day_number'>
							<input checked type='radio' name='month_type' id='number_radio' />
							<label for='number_radio'>
								<span id='repeat_month_date' ></span> du mois
							</label>
						</div>
						<div id='repeat_all_month_by_day'>
							<input type='radio' name='month_type' id='day_of_week_radio' />
							<label for='day_of_week_radio'>
								<span id='repeat_month_day_number'></span>
								<span id='repeat_month_day'></span>
								du mois
							</label>
						</div>
					...
					<div id='repeat_all_day'>Tous les <input type='text' maxlenght="2" value="1" size="2" id='repeat_day_delay' /> jour(s)</div>
					Créer au total <input type='text' value="5" size="2" id='number_of_repeat' /> évènements.
				</div>
				<?php }else echo "<em>Il n'est pas possible d'ajouter une récurrence sur un évènement existant</em>"; ?>
			</td>	
...
<tr>
   <th>Lieu</th>
   <td colspan="4">
<input type='text' id='eve_lieu' name='eve_lieu' size="50" <?php if(!$droit_modif){echo 'disabled';} ?> value="<?php echo $eve['eve_lieu']; ?>" />
   </td>
</tr>
...
?>
	...
	<td colspan="3">
<?php  if ($eve['eve_date_modification']) { 
    echo 'le '.substr ($eve['eve_date_modification'], 0, 16);
  } ?>
	</td>
      </tr>
  <?php  if (count ($categories_evs) == 1) : ?>
...
function liste_participants () {
  ...
  foreach ($entites as $entite) {
    ...
    if ($droit_modif){
      ...
      if($entite['ent_code'] === 'personnel'){
          echo '<div class="multiple_add">';
          echo '<span class="multiple_add_libelle" id="ajout_participant_select" onclick="$(\'#multiple_add_personnel,.multiple_add_libelle\').toggle()">Ajouter...</span>';
          echo '<div class="multiple_add_menu" id="multiple_add_personnel">';
          echo '<span class="ajout_participant" id="ajout_participant_personnel" onclick="$(\'#multiple_add_personnel,.multiple_add_libelle\').toggle()">Un collègue</span>';
          echo '<span id="ajout_participant_myself" onclick="$(\'#multiple_add_personnel,.multiple_add_libelle\').toggle()">Moi-même</span>';
          echo '</div></div>';
      }
      else{
