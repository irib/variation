...
$(document).ready (function () {
    $("#dty_id").change (on_dty_id_change);
    $(".doc_statut").hide ();
...
function on_dty_id_change () {
  var dty_id = $(this).val();
  $.getJSON ('/ajax/edit/document_document_type_get.php', { prm_token: $("#token").val(), prm_dty_id: dty_id, output: 'json2' }, function (data) {
      if (!data.dty_nom_individuel && $("#doc_titre").val() == '') {
	$("#doc_titre").val (data.dty_nom);
      }
    });
}
...
function on_doc_save_click () {
    ...
    if (!$("#dty_id").val().length) {
	alert ('Vous devez indiquer un type');
	return;	
    }
...
function affiche_type ($val) {
  global $doc, $base, $secteurs_dos, $dos_id;
  ...
  if ($doc) {
    ...
    if (count ($secteurs_dos) == 1) {
	  ...
	  $info_doc = $base->document_documents_get($_SESSION['token'], $dos_id);
    }
  }
  if(empty($info_doc['dty_id'])){
  ...
  foreach ($dtys as $dty) {
	...
	else if($info_doc['dty_id']==$dty['dty_id']){
		echo '<option selected value="'.$dty['dty_id'].'"'.$selected.'>'.$dty['dty_nom'].'</option>';  
	}
  }
}
