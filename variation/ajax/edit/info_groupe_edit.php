...
<?php
/* 
   Ce dialogue permet d'éditer les affectations des usagers à des groupes.

   I. Édition d'une affectation existante :
    - ing_id: Champ d'affectation 
    - peg_id: Affectation à éditer

    II. Ajout d'une affectation :
    - ing_id: Champ d'affectation
    - per_id: Personne à affecter
 */
...
var sec_editable = false;
$(document).ready (function () {

    $("#eta_ajout").hide();

    $.getJSON ("/ajax/secteur_infos_get.php", { prm_token: $("#token").val(), prm_sec_id: <?= $secteur1['sec_id'] ?>, output: 'json2' }, function (ret) {
	sec_editable = ret.sec_editable;
	if (sec_editable)
	  $("#eta_ajout").show();
	else
	  $("#eta_ajout").hide();
    ...
    $("#grp_ajout").hide();
    on_peg_eta_id_change();
    $("#peg_eta_id").change (on_peg_eta_id_change);

    $("#eta_ajout").click (on_eta_ajout_click);
    $("#grp_ajout").click (on_grp_ajout_click);
});

function on_peg_eta_id_change() {
  ...
  $.getJSON ("/ajax/etablissement_get.php", { prm_token: $("#token").val(), prm_eta_id: eta_id, output: 'json2' }, function (ret) {
      if (ret.eta_id) {
	if (ret.cat_id) {	  
	  $.getJSON ("/ajax/etablissement_secteur_edit_get.php", { prm_token: $("#token").val(), prm_eta_id: eta_id, prm_secteur: $("#sec_code").val(), output: 'json2' }, function (ret) {
	      if (ret)
		$("#grp_ajout").show();
	      else
		$("#grp_ajout").hide();
  ...
  $.post ('/ajax/edit/groupe_liste.php', {
	...
	$.each (data, function (idx, val) {
	    $("#peg_grp_id").append ('<option '+(val.grp_id == grp_id ? ' selected ' : '')+'value="'+val.grp_id+'">'+val.grp_nom+'</option>');
	  });
      }
    });
}
...
function on_peg_save_click () {
    ...
    if($("#peg_fin").val()!="")if(!check_periode($('#peg_debut').val(), $('#peg_fin').val())){
...
<input type="hidden" id="grp_id" value="<?= $peg['grp_id'] ?>"></input>
