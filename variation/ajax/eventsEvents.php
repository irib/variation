...
// Evenements
$eves = $base->events_event_liste ($_SESSION['token'], $evs['evs_id'], $per_id, $startDate, $endDate, $grp_id, $per_ids, $_SESSION['eta_id']);
if (count ($eves)) {
  foreach ($eves as $eve) {
    ...
    if ($fromAndroid) {
      $events[] = array ("title" => $titre,
        ...
        $color = "#3D84CC";
        if(!empty($eve['sec_couleur'])){
            $color = $eve['sec_couleur'];
        }
      $events[] = array ("title" => $titre."\n".$listusagers,
