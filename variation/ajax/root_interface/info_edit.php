...
$info = $base->meta_info_get ($_SESSION['token'], $_GET['id']);
$ina_aide = $base->meta_info_aide_get ($_SESSION['token'], $info['inf_id']);
...
$selections = $base->meta_selection_liste ($_SESSION['token']);
...
$(document).ready (function () {
    $.getJSON('/ajax/meta_infos_type_liste.php?output=json', { prm_token: $("#token").val()}, function (data) {
...
<input type="hidden" id="dirid" value="<?= arrval ($_GET, 'dirid', '') ?>"></input>
<table width="100%" cellpadding="5">
  ...
  <tr>
    <td>Code&nbsp;:</td><td><input type="text" id="inf_code" size="30" value="<?= $info['inf_code'] ?>" <?= $_GET['id'] != 0 ? 'readonly' : '' ?>></input></td>
  </tr>
  <tr>
