...
$infos_inutilises = $base->meta_info_inutilises($_SESSION['token']);
$inutilises = array ();
foreach ($infos_inutilises as $tmp) {
  $inutilises[$tmp['inf_id']] = true;
}
$ret = array ();
if (is_array ($infos)) {
    ...
    if (!$info['din_id']) {
      $ret[] = array ('data' => $info['inf_libelle'].' ('.$int[$info['int_id']]['int_libelle'].')',
		      'attr' => array ('myid' => 'inf_'.$info['inf_id'], 
				       'class' => isset ($inutilises[$info['inf_id']]) ? 'unused' : '',
				       'id' => 'inf_'.$info['inf_id'], 
