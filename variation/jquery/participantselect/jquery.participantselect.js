...
(function ($) {
	...
	var defaults = {
      ...
      'por_id':null,//Utiliser pour ajout
      'per_id':null,//id usager pour filtre sur lien avec l'usager
      'eta_id':null,//Utiliser pour ajout
	    'return': null,
	...
	return this.each (function () {
	    ...
	    div.empty().html('<div id="participantselect-frame"><div id="participantselect-typeper">('+o.eta_id+')</div></div>');
	    var i = 0;
	    ...
	    $('#participantselect-frame').append ('<br/><label class="etiq" for="participantselect-nom">Nom : </label><input type="text" onchange="$(\'#participantselect-chercher\').click();" id="participantselect-nom"><br/><label class="etiq" for="participantselect-prenom">Prénom : </label><input onchange="$(\'#participantselect-chercher\').click();" type="text" id="participantselect-prenom">');
	    
      if(o.per_id!=null && o.per_id!="" && o.type.indexOf('usager')== -1){
        $('#participantselect-frame').append ('<br/><input checked type="checkbox" id="en_lien_uniquement" />&nbsp;<label for="en_lien_uniquement">Personnes en lien avec l\'usager uniquement</label>');
      }
            if ($('#participantselect-typeper label').first().text()==='usager'){
                $('#participantselect-frame').append('<br/><label class="etiq" for="participantselect-group">Groupe :</label><select name="rech_grp_id" onchange="$(\'#participantselect-chercher\').click();" id="participantselect-group"></select>');
	    ...
	    if (o.ajout) {
		$("#participantselect-buttons").append('<button style="display:none" id="participantselect-ajout">Ajouter</button><div id="dlg_ajout"></div>');
	    }
		...
		var type = $("input[name=participantselect-type]:checked").val();
		$('#participantselect-ajout').show();
    
    var en_lien_uniquement=false;
    var grp_id = $("#participantselect-group").val();
    if(grp_id==null){
      grp_id="";
    }
    ...
    if(o.secteur != null){
      url_recherche = "/ajax/personne_cherche2.php";
    }
    
    
    
    if(o.per_id!=null && o.per_id!="" && o.type.indexOf('usager')== -1){
      en_lien_uniquement = $('#en_lien_uniquement').is(':checked');
      if(en_lien_uniquement){
        args.prm_per_id = o.per_id;
        url_recherche = "/ajax/personne_cherche_avec_lien.php";
        if(o.secteur != null){
          url_recherche = "/ajax/personne_cherche2_avec_lien.php";
        }
      }
    }
		$.getJSON (url_recherche, args , function (data) {
      ...
      if(o.per_id!=null && o.per_id!="" && o.type.indexOf('usager')== -1){
		...
		$("#participantselect-ajout").button ({icons: {primary:'ui-icon-plus'}}).click (function () {
		    // Appel interface d'ajout	
			var type = $("input[name=participantselect-type]:checked").val();
			 $("#dlg_ajout").personneAjout({
				"por_id": o.por_id, 
				"eta_id": o.eta_id,
				"type": type,
				"droit": true,
	    ...
	    div.dialog ({
			...
			$(div).unbind('dialogclose').bind('dialogclose',function(event){
