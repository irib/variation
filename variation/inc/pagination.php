...
class Pagination {
  ...
  function addColumn ($label, $code = null, $link_function = null, $link_id = null, $tag = null) {
    $this->ncolumns ++;
    $this->columns[$this->ncolumns] = array ($label, $code, $link_function, $link_id, $tag);
  }
  ...
  function getData () {
    $args = $this->args;
    $p = arrval ($_GET, 'p', '1');
    $col = arrval ($_GET, 'col', $this->defaultColOrder);
  ...
  function displayData ($cols) {
	...
	foreach ($cols as $col) {
      ...
      if ($this->columns[$i][4] === null) {
          echo $dat[$col];
      }
      else if($this->columns[$i][4] === 'color'){
          echo "<span class='color_picker' style='background-color:".$dat[$col]."'></span>";
      }
      else {
          echo "ERREUR";
      }
  ...
  function link_page ($p) {
    return $this->url_file.'?'.http_build_query (array_merge ($this->url_args,
							      array ('col' => arrval ($_GET, 'col'),
								     'sort' => arrval ($_GET, 'sort'),
								     'p' => $p
								     )));
  }
