...
function exporte_portail_csv ($token, &$csv, $portail) {
  global $base;
  $ents = array ();
  $ents[] = array ('code' => 'usager',
		   'libelle' => 'Usager');
  ...
  $ents[] = array ('code' => 'famille',
		   'libelle' => 'Famille');
  $ligne = array ('portail', $portail['por_libelle']);
  $dajs = $base->droit_ajout_entite_portail_liste_par_portail ($token, $portail['por_id']);
  $droits = array ();
  foreach ($dajs as $daj) {
    $droits[$daj['ent_code']] = $daj['daj_droit'];
  }
  $ligne[] = $droits['usager'] ? '1' : '';
  $ligne[] = $droits['personnel'] ? '1' : '';
  $ligne[] = $droits['contact'] ? '1' : '';
  $ligne[] = $droits['famille'] ? '1' : '';
  $droits_note = $base->droit_portail_get($token, $portail['por_id'], 'suppression_notes');
  $ligne[] = $droits_note ? '1' : '';
  $csv->addLine(array('#PORTAIL', 'NOM', 'AJOUT USAGER', 'AJOUT PERSONNEL', 'AJOUT CONTACT', 'AJOUT FAMILLE', 'SUPPRESSION NOTES'));
  $csv->addLine ($ligne);
  foreach ($ents as $ent) {
    exporte_entite_csv ($token, $csv, $portail, $ent);
  }
  exporte_principal_csv ($token, $csv, $portail);
}
...
function exporte_entite_csv ($token, &$csv, $portail, $ent) {
  global $base;
  $ligne = array ('fenetre', $ent['code']);
  $csv->addLine (array ('#FENETRE', 'CODE'));
  $csv->addLine ($ligne);
  $menus = $base->meta_menu_liste ($token, $portail['por_id'], $ent['code']);
  if (count ($menus)) {
    foreach ($menus as $menu) {
      exporte_menu_csv ($token, $csv, $menu);
    }
  }
}
...
function exporte_menu_csv ($token, &$csv, $menu) {
  global $base;
  $ligne = array ('menu', $menu['men_libelle']);
  $csv->addLine (array('#MENU', 'NOM'));
  $csv->addLine ($ligne);
  $sousmenus = $base->meta_sousmenu_liste ($token, $menu['men_id']);
  if (count ($sousmenus)) {
    foreach ($sousmenus as $sousmenu) {
      exporte_sousmenu_csv ($token, $csv, $sousmenu);
    }
  }
}
...
function exporte_sousmenu_csv ($token, &$csv, $sousmenu) {
  global $base;
  $ligne = array('sousmenu', $sousmenu['sme_type']); // parmi: notes, documents, event, '' (pour masque de données) 
  $code = '';
  switch ($sousmenu['sme_type']) {
  case 'notes':
    $nos = $base->notes_notes_get ($token, $sousmenu['sme_type_id']);
  ...
  $ligne[] = $code;
  $ligne[] = $sousmenu['sme_droit_modif'];
  $ligne[] = $sousmenu['sme_droit_suppression'];
  $ligne[] = $sousmenu['sme_libelle'];
  $ligne[] = $sousmenu['sme_icone'];
  $csv->addLine (array('#SOUSMENU', 'TYPE', 'CODE', 'MODIF', 'SUPPRESSION', 'NOM', 'ICONE'));
  $csv->addLine ($ligne);

  $groupes = $base->meta_groupe_infos_liste ($token, $sousmenu['sme_id']);
  if (count ($groupes)) {
    foreach ($groupes as $groupe) {
      $ligne = array ('bloc', $groupe['gin_libelle']);
      $csv->addLine (array('#BLOC', 'NOM'));
      $csv->addLine ($ligne);
      $infos = $base->meta_info_groupe_liste ($token, $groupe['gin_id']);
      if (count ($infos)) {
	$csv->addLine (array('#CHAMP', 'CODE', 'CYCLE', 'OBLIGATOIRE'));
	foreach ($infos as $info) {
	  $ligne = array ('champ', $info['inf_code'], $info['ing__groupe_cycle'], $info['ing_obligatoire']);
	  $csv->addLine ($ligne);
	}
      }
    }
...
function exporte_principal_csv ($token, &$csv, $portail) {
  global $base;
  $ligne = array ('fenetre', 'principale');
  $csv->addLine (array('#FENETRE', 'CODE'));
  $csv->addLine ($ligne);
  $topmenus = $base->meta_topmenu_liste ($token, $portail['por_id']);
  if (count ($topmenus)) {
    foreach ($topmenus as $topmenu) {
      exporte_topmenu_csv ($token, $csv, $topmenu);
    }
  }
}
...
function exporte_topmenu_csv ($token, &$csv, $topmenu) {
  global $base;
  $ligne = array ('menu', $topmenu['tom_libelle']);
  $csv->addLine (array('#MENU', 'NOM'));
  $csv->addLine ($ligne);
  $topsousmenus = $base->meta_topsousmenu_liste ($token, $topmenu['tom_id']);
  if (count ($topsousmenus)) {
    $csv->addLine (array('#SOUSMENU', 'TYPE', 'CODE', 'MODIF', 'SUPPRESSION', 'NOM', 'ICONE', 'TITRE'));
    foreach ($topsousmenus as $topsousmenu) {
      $ligne = array ('sousmenu', $topsousmenu['tsm_type']); //  notes, documents, liste, groupe, event, ressource, affectations
      $code = '';
      switch ($topsousmenu['tsm_type']) {
      case 'notes':
	$nos = $base->notes_notes_get ($token, $topsousmenu['tsm_type_id']);
	...
	break;
      case 'affectations':
	$aff = $base->vues_affectations_get  ($token, $topsousmenu['tsm_type_id']);
	$code = $aff['aff_code'];
	break;
      }
      $ligne[] = $code;

      $ligne[] = $topsousmenu['tsm_droit_modif'];
      $ligne[] = $topsousmenu['tsm_droit_suppression'];
      $ligne[] = $topsousmenu['tsm_libelle'];
      $ligne[] = $topsousmenu['tsm_icone'];
      $ligne[] = $topsousmenu['tsm_titre'];
      $csv->addLine ($ligne);
      // TODO : sme_id_lien_usager 
    }
  }
