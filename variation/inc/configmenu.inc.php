...
class ConfigMenu {
  ...
  public function display ($isRoot, $isAdmin) {
    if ((!$isRoot && $this->isRoot) || (!$isAdmin && $this->isAdmin))
      return;
    echo '<li'.($this->script == arrval ($_SESSION, 'config') ? ' class="selected"' : '').'><a href="?config='.$this->script.'"><span class="icone"><img src="'.$this->icon.'"></img></span><span class="label">'.$this->label.'</span></a></li>';
  }
}

if (!isset ($config_exportarr))
  $config_exportarr = false;
...
$configmenus = array (
		      ...
		      new TopConfigMenu ("Vues",
					 array (/*new ConfigMenu ("root/vues_listes", "Vues listes",
						...
						new ConfigMenu ("root/vues_listes", "Vues de listes",
		      ...
		      new TopConfigMenu ("Outils",
					 array (new ConfigMenu ("root/imports", "Imports",
								"/Images/IS_Real_vista_Database/originals/png/NORMAL/32/insert_table_32.png"),
						new ConfigMenu ("root/debug", "Qualité des données",
								"/Images/IS_Real_vista_Social/originals/png/NORMAL/32/report_bug_32.png"),
						$config_exportarr ? new ConfigMenu ("root/exportarr", "Export arrangement",
										    "/Images/IS_Real_vista_3d_graphics/originals/png/NORMAL/32/gear_32.png")
						: null,
						)),
		      );
?>
