...
DROP FUNCTION IF EXISTS utilisateur_login2(prm_login character varying, prm_mdp character varying);
DROP TYPE IF EXISTS utilisateur_login2;
...
DROP TYPE IF EXISTS utilisateur_liste_details_configuration;
...
BEGIN
	...
	FOR row IN
		SELECT uti_id, uti_login, personne_info_varchar_get (prm_token, per_id, 'prenom'), personne_info_varchar_get (prm_token, per_id, 'nom'), uti_email FROM login.utilisateur
...
DROP TYPE IF EXISTS utilisateur_usagers_liste;
...
DROP FUNCTION IF EXISTS utilisateur_add(prm_token integer, prm_login character varying, prm_per_id integer, prm_uti_config boolean, prm_uti_root boolean);
CREATE OR REPLACE FUNCTION utilisateur_add(prm_token integer, prm_login character varying, prm_per_id integer, prm_uti_config boolean, prm_uti_root boolean, prm_email text) RETURNS integer
    LANGUAGE plpgsql
    AS $$
DECLARE
	ret integer;
...
END;
$$;
COMMENT ON FUNCTION utilisateur_add(prm_token integer, prm_login character varying, prm_per_id integer, prm_uti_config boolean, prm_uti_root boolean, prm_email text) IS 
...
Entrée :
 ...
 - prm_email : Email de l''utilisateur (pour caldav)
Retour :
...
DROP FUNCTION IF EXISTS utilisateur_get_par_per_id(prm_per_id integer);
CREATE OR REPLACE FUNCTION utilisateur_get_par_per_id(prm_token integer, prm_per_id integer) RETURNS utilisateur
    LANGUAGE plpgsql
    AS $$
DECLARE
	ret login.utilisateur;
BEGIN
	PERFORM login._token_assert (prm_token, FALSE, FALSE);
	SELECT utilisateur.* INTO ret FROM login.utilisateur WHERE per_id = prm_per_id ORDER BY uti_id LIMIT 1;
	RETURN ret;
END;
$$;
COMMENT ON FUNCTION utilisateur_get_par_per_id(prm_token integer, prm_per_id integer) IS
'Retourne les informations de l''utilisateur ayant un certain per_id (considère qu''il existe un seul utilisateur par per_id et retourne le premier)';
...
DROP FUNCTION IF EXISTS utilisateur_update(prm_token integer, prm_uti_id integer, prm_login character varying, prm_per_id integer, prm_uti_config boolean, prm_uti_root boolean);
CREATE OR REPLACE FUNCTION utilisateur_update(prm_token integer, prm_uti_id integer, prm_login character varying, prm_per_id integer, prm_uti_config boolean, prm_uti_root boolean, prm_email text) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
	PERFORM login._token_assert (prm_token, TRUE, FALSE);
	UPDATE login.utilisateur SET uti_login = prm_login, per_id = prm_per_id, uti_config = prm_uti_config, uti_root = prm_uti_root, uti_email = prm_email WHERE uti_id = prm_uti_id;
END;
$$;
COMMENT ON FUNCTION utilisateur_update(prm_token integer, prm_uti_id integer, prm_login character varying, prm_per_id integer, prm_uti_config boolean, prm_uti_root boolean, prm_email text) IS
...
CREATE OR REPLACE FUNCTION utilisateur_update_profil(prm_token integer, prm_uti_id integer, prm_email text) RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
	PERFORM login._token_assert (prm_token, FALSE, FALSE);
	UPDATE login.utilisateur SET uti_email = prm_email WHERE uti_id = prm_uti_id;
END;
$$;
COMMENT ON FUNCTION utilisateur_update_profil(prm_token integer, prm_uti_id integer, prm_email text) IS
