...
CREATE OR REPLACE FUNCTION droit_portail_get(prm_token integer, prm_por_id integer, prm_droit varchar)
RETURNS BOOLEAN
    LANGUAGE plpgsql
    AS $$
DECLARE
...
BEGIN
  PERFORM login._token_assert (prm_token, FALSE, TRUE);
  SELECT drp_valeur INTO ret FROM permission.droit_portail WHERE por_id = prm_por_id AND drp_droit = prm_droit;
  IF NOT FOUND THEN 
    ret = false;
  END IF;
  RETURN ret;
...
CREATE OR REPLACE FUNCTION droit_portail_set(prm_token integer, prm_por_id integer, prm_droit varchar, prm_valeur BOOLEAN)
RETURNS void
    LANGUAGE plpgsql
    AS $$
BEGIN
  PERFORM login._token_assert (prm_token, FALSE, TRUE);
  UPDATE permission.droit_portail SET drp_valeur = prm_valeur WHERE por_id = prm_por_id AND drp_droit = prm_droit;
  IF NOT FOUND THEN
    INSERT INTO permission.droit_portail (por_id, drp_droit, drp_valeur) VALUES (prm_por_id, prm_droit, prm_valeur);
  END IF;
END;
$$;
