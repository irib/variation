...
DROP TYPE IF EXISTS note_destinataire_derniers_par_utilisateur;
...
DROP TYPE IF EXISTS note_destinataires_liste;
...
DROP TYPE IF EXISTS note_usagers_liste;
...
DROP TYPE IF EXISTS notes_note_boite_envoi_liste;
...
DROP TYPE IF EXISTS notes_theme_liste_details;
...
BEGIN
	...
	FOR row IN 
		SELECT DISTINCT the_id, the_nom, concatenate (por_libelle) FROM notes.theme 
			LEFT JOIN notes.theme_portail USING(the_id) 
			LEFT JOIN meta.portail USING(por_id) 
...
DROP FUNCTION IF EXISTS notes_note_usager_liste(prm_token integer, prm_per_id integer, prm_nos_id integer);
CREATE OR REPLACE FUNCTION notes_note_usager_liste(prm_token integer, prm_per_id integer, prm_nos_id integer, prm_start date, prm_end date) RETURNS SETOF note
    LANGUAGE plpgsql
    AS $$
DECLARE
	the integer;
...
END;
$$;
COMMENT ON FUNCTION notes_note_usager_liste(prm_token integer, prm_per_id integer, prm_nos_id integer, prm_start date, prm_end date) IS
