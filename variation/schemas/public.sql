...
DROP TYPE IF EXISTS contact_recherche;
...
DROP TYPE IF EXISTS etablissement_liste_details;
...
DROP TYPE IF EXISTS famille_recherche;
...
DROP TYPE IF EXISTS groupe_liste;
...
DROP FUNCTION IF EXISTS groupe_liste_details(prm_token integer, prm_eta_id integer, prm_sec_id_role integer, prm_sec_id_besoin integer, prm_interne_seuls boolean);
DROP TYPE IF EXISTS groupe_liste_details;
...
DROP TYPE IF EXISTS personne_cherche;
...
BEGIN
	PERFORM login._token_assert (prm_token, FALSE, FALSE);
	RAISE WARNING 'personne_cherche 5x';
	FOR row IN
		SELECT DISTINCT per_id, 
		COALESCE (personne_info_varchar_get(prm_token, per_id, 'nom') || ' ', '') || COALESCE (personne_info_varchar_get(prm_token, per_id, 'prenom'), '') ||
		(CASE WHEN prm_type = 'usager' THEN ''
...
CREATE OR REPLACE FUNCTION personne_en_lien(prm_token integer, prm_per_id integer, prm_per_id_lien integer)
RETURNS BOOLEAN 
LANGUAGE plpgsql
AS $$
BEGIN
  RETURN EXISTS(SELECT 1 FROM public.personne_info
  	 	  inner join personne_info_integer using(pin_id)
		  inner join meta.info using(inf_code)
  		  inner join meta.infos_type using(int_id)
  		  where int_code = 'contact' and per_id = prm_per_id and pii_valeur = prm_per_id_lien)
	OR EXISTS(SELECT 1 FROM personne_info_lien_familial 
	   	  inner join personne_info using(pin_id) 
		  where per_id = prm_per_id and per_id_parent = prm_per_id_lien);
END;
$$;
COMMENT ON FUNCTION personne_en_lien(prm_token integer, prm_per_id integer, prm_per_id_lien integer) IS
'Retourne true si une personne est en lien avec un usager (lien familial ou champ lien)';

CREATE OR REPLACE FUNCTION personne_lien_familial_intitule(prm_token integer, prm_per_id integer, prm_per_id_parent integer)
RETURNS varchar
LANGUAGE plpgsql
AS $$
DECLARE
  ret varchar;
BEGIN
  SELECT lfa_nom INTO ret FROM meta.lien_familial
    INNER JOIN personne_info_lien_familial USING(lfa_id)
    INNER JOIN personne_info USING(pin_id)
    WHERE per_id = prm_per_id AND per_id_parent = prm_per_id_parent
    LIMIT 1;
  RETURN ret;
END;
$$;
COMMENT ON FUNCTION personne_lien_familial_intitule(prm_token integer, prm_per_id integer, prm_per_id_parent integer) IS
'Retourne l''intitulé du lien familial entre l''usager et le membre de la famille donnés';

CREATE OR REPLACE FUNCTION personne_cherche_avec_lien(prm_token integer, prm_nom character varying, prm_prenom character varying, prm_type character varying, prm_grp_id integer, prm_per_id integer) RETURNS SETOF personne_cherche
    LANGUAGE plpgsql
    AS $$
DECLARE
	row personne_cherche;
BEGIN
	PERFORM login._token_assert (prm_token, FALSE, FALSE);
	RAISE WARNING 'personne_cherche_avec_lien 6x';
	FOR row IN
		SELECT DISTINCT per_id, 
		COALESCE (personne_info_varchar_get(prm_token, per_id, 'nom') || ' ', '') || COALESCE (personne_info_varchar_get(prm_token, per_id, 'prenom'), '') ||
		(CASE WHEN prm_type = 'usager' THEN ''
		     WHEN prm_type = 'famille' THEN COALESCE (' (' || personne_lien_familial_intitule(prm_token, prm_per_id, per_id) || ')', '')
		     WHEN prm_type = 'contact' THEN COALESCE (' (' || (SELECT met_nom FROM meta.metier WHERE met_id = (SELECT * FROM personne_info_integer_get(prm_token, per_id, 'contact_metier'))) || ')', '')
		     WHEN prm_type = 'personnel' THEN COALESCE (' (' || (SELECT met_nom FROM meta.metier WHERE met_id = (SELECT * FROM personne_info_integer_get(prm_token, per_id, 'personnel_metier'))) || ')', '')  
		 END)
		FROM personne 
	...
	LOOP
		IF personne_en_lien(prm_token, prm_per_id, row.per_id) THEN
		   RETURN NEXT row;
		END IF;
	END LOOP;
END;
...
CREATE OR REPLACE FUNCTION personne_cherche2_avec_lien(prm_token integer, prm_nom character varying, prm_prenom character varying, prm_type character varying, prm_secteur character varying, prm_per_id integer) RETURNS SETOF personne_cherche
    LANGUAGE plpgsql
    AS $$
DECLARE
	row personne_cherche;
BEGIN
--	RAISE WARNING '%', prm_secteur;
	-- RAJOUTER LE LIEN FAMILIAL, NE SERT PAS POUR L'INSTANT
	PERFORM login._token_assert (prm_token, FALSE, FALSE);
	FOR row IN
		SELECT DISTINCT per_id, 
		COALESCE (personne_info_varchar_get(prm_token, per_id, 'nom') || ' ', '') || COALESCE (personne_info_varchar_get(prm_token, per_id, 'prenom'), '') ||
...
DROP TYPE IF EXISTS personne_contact_liste;
...
DROP TYPE IF EXISTS personne_info_boolean_histo;
...
DROP TYPE IF EXISTS personne_info_contact_histo;
...
DROP TYPE IF EXISTS personne_info_date_histo;
...
DROP FUNCTION IF EXISTS personne_info_integer_get_details(prm_token integer, prm_per_id integer, prm_inf_code character varying);
DROP TYPE IF EXISTS personne_info_integer_get_multiple_details;
...
DROP TYPE IF EXISTS personne_info_integer_histo;
...
DROP TYPE IF EXISTS personne_info_varchar_histo;
...
DROP TYPE IF EXISTS pgprocedures_search_arguments;
...
DROP TYPE IF EXISTS pgprocedures_search_function;
...
DROP TYPE IF EXISTS prise_en_charge_select;
...
BEGIN
	PERFORM login._token_assert (prm_token, FALSE, FALSE);
	FOR row IN
		SELECT DISTINCT secteur.* FROM meta.secteur
...
Entrées :
 - prm_token : Token d''authentification
 - prm_eta_id : Identifiant de l''établissement/partenaire, NULL pour tous
...
DROP FUNCTION IF EXISTS groupe_affectation_liste(prm_token integer, prm_inf_id integer);
DROP TYPE IF EXISTS groupe_affectation_liste;
CREATE TYPE groupe_affectation_liste AS (
  grp_id integer,
  grp_nom varchar,
  eta_id integer,
  eta_nom varchar,
  grp_capacite integer
);
CREATE OR REPLACE FUNCTION groupe_affectation_liste(prm_token integer, prm_inf_id integer) RETURNS SETOF groupe_affectation_liste
    LANGUAGE plpgsql
    AS $$
DECLARE
	row groupe_affectation_liste;
BEGIN
	PERFORM login._token_assert (prm_token, FALSE, FALSE);
	FOR row IN
		SELECT DISTINCT groupe.grp_id, groupe.grp_nom, etablissement.eta_id, etablissement.eta_nom, groupe.grp_capacite
		FROM groupe 
		INNER JOIN groupe_secteur USING (grp_id)
		INNER JOIN meta.secteur ON secteur.sec_id = groupe_secteur.sec_id
		INNER JOIN meta.info ON info.inf__groupe_type = secteur.sec_code
		INNER JOIN groupe_info_secteur ON groupe_info_secteur.grp_id=groupe.grp_id AND groupe_info_secteur.inf_id=info.inf_id
		LEFT JOIN etablissement ON etablissement.eta_id = groupe.eta_id
		where groupe_info_secteur.inf_id = prm_inf_id
		ORDER BY grp_nom
	LOOP
		return NEXT row;
	END LOOP;
END;
$$;
COMMENT ON FUNCTION groupe_affectation_liste(prm_token integer, prm_inf_id integer) IS
'Retourne la liste des groupes associés à un champ donné.';

DROP FUNCTION IF EXISTS groupe_affectation_nombre_par_statut(prm_token integer, prm_grp_id integer, prm_statut integer);
DROP TYPE IF EXISTS groupe_affectation_nombre_par_statut;
CREATE TYPE groupe_affectation_nombre_par_statut AS (
  n integer
);
CREATE FUNCTION groupe_affectation_nombre_par_statut(prm_token integer, prm_grp_id integer, prm_statut integer)
RETURNS SETOF groupe_affectation_nombre_par_statut
LANGUAGE plpgsql
AS $$
DECLARE
  row groupe_affectation_nombre_par_statut;
BEGIN
  FOR row IN
    SELECT count(DISTINCT per_id) FROM personne_groupe
      WHERE 
        grp_id = prm_grp_id 
 	AND peg_cycle_statut = prm_statut
	AND CURRENT_TIMESTAMP BETWEEN COALESCE(peg_debut, '-Infinity'::timestamp) AND COALESCE(peg_fin, 'Infinity'::timestamp)
  LOOP
    RETURN NEXT row;
  END LOOP;
END;
$$;
COMMENT ON FUNCTION groupe_affectation_nombre_par_statut(prm_token integer, prm_grp_id integer, prm_statut integer) IS 'Retourne le nombre de personnes affectées à un groupe avec un statut d''usager donné.';

CREATE OR REPLACE FUNCTION groupe_affectation_noms_par_statut(prm_token integer, prm_grp_id integer, prm_statut integer)
...
END;
$$;
COMMENT ON FUNCTION groupe_affectation_noms_par_statut(prm_token integer, prm_grp_id integer, prm_statut integer) IS 'Retourne les noms des personnes affectées à un groupe avec un statut d''usager donné.';

CREATE OR REPLACE FUNCTION groupe_affectation_nombre_presents_a_date(prm_token integer, prm_grp_id integer, prm_date date)
RETURNS integer
LANGUAGE plpgsql
AS $$
DECLARE
  ret integer;
BEGIN
  SELECT count(DISTINCT per_id) INTO ret FROM personne_groupe
    WHERE 
      grp_id = prm_grp_id 
      AND prm_date BETWEEN COALESCE(peg_debut, '-Infinity'::timestamp) 
                       AND COALESCE(peg_fin, 'Infinity'::timestamp)
      AND peg_cycle_statut IN (2, 3);
      -- et statut actuel present ou termine
  RETURN ret;
END;
$$;
COMMENT ON FUNCTION groupe_affectation_nombre_presents_a_date(prm_token integer, prm_grp_id integer, prm_date date) IS 'Retourne le nombre de personnes affectées à un groupe à une date donnée.';

CREATE OR REPLACE FUNCTION groupe_affectation_noms_presents_a_date(prm_token integer, prm_grp_id integer, prm_date date)
RETURNS SETOF varchar
LANGUAGE plpgsql
AS $$
DECLARE
...
END;
$$;
COMMENT ON FUNCTION groupe_affectation_noms_presents_a_date(prm_token integer, prm_grp_id integer, prm_date date) IS 'Retourne les noms de personnes affectées à un groupe à une date donnée.';
...
CREATE OR REPLACE FUNCTION groupe_set_capacite(prm_token integer, prm_grp_id integer, prm_capacite integer) RETURNS boolean
    LANGUAGE plpgsql
    AS $$
BEGIN
	PERFORM login._token_assert (prm_token, FALSE, FALSE);
	UPDATE groupe SET grp_capacite = prm_capacite WHERE grp_id = prm_grp_id;
	RETURN FOUND;
END;
$$;
COMMENT ON FUNCTION groupe_set_capacite(prm_token integer, prm_grp_id integer, prm_capacite integer) IS
'Modifie la capacité d''un groupe d''usagers.

Entrées :
 - prm_token : Token d''authentification
 - prm_grp_id : Identifiant du groupe à mettre à jour
 - prm_capacite : Nouvelle capacité du groupe
';
...
CREATE OR REPLACE FUNCTION personne_info_integer_get(prm_token integer, prm_per_id integer, prm_inf_code character varying) RETURNS int
    LANGUAGE plpgsql
    AS $$
DECLARE
	ret integer;
...
CREATE FUNCTION personne_info_integer_get_details(prm_token integer, prm_per_id integer, prm_inf_code character varying) RETURNS personne_info_integer_get_multiple_details
    LANGUAGE plpgsql
    AS $$
DECLARE
	ret personne_info_integer_get_multiple_details;
BEGIN
	PERFORM login._token_assert (prm_token, FALSE, FALSE);
	SELECT pii_id, pii_valeur INTO ret 
		FROM personne_info_integer
		INNER JOIN personne_info USING (pin_id)
		WHERE per_id = prm_per_id AND inf_code = prm_inf_code
		ORDER BY pii_id DESC LIMIT 1;
...
DROP FUNCTION IF EXISTS personne_info_text_get_histo(prm_token integer, prm_per_id integer, prm_inf_code character varying);
DROP TYPE IF EXISTS personne_info_text_histo;
CREATE TYPE personne_info_text_histo AS (
	debut date,
	fin date,
	valeur text,
	utilisateur varchar
...
CREATE OR REPLACE FUNCTION personne_info_text_get_histo(prm_token integer, prm_per_id integer, prm_inf_code character varying) RETURNS SETOF personne_info_text_histo
    LANGUAGE plpgsql
    AS $$
DECLARE
	row personne_info_text_histo;
BEGIN
	PERFORM login._token_assert (prm_token, FALSE, FALSE);
	FOR row IN
		SELECT pit_debut, pit_fin, regexp_replace(pit_valeur, '\r|\n', '<br>', 'g'),
		       login.utilisateur_prenon_nom(prm_token, uti_id) 
		FROM personne_info_text
		INNER JOIN personne_info USING (pin_id)
		WHERE per_id = prm_per_id AND inf_code = prm_inf_code
...
BEGIN
	...
	-- Valeur historisée :
		...
		IF NOT FOUND THEN
			INSERT INTO personne_info (per_id, inf_code) VALUES (prm_per_id, prm_inf_code) RETURNING pin_id INTO the_pin_id;
		END IF;
		SELECT pit_id INTO pit_id_dernier FROM personne_info_text WHERE pin_id = the_pin_id
			ORDER BY pit_id DESC LIMIT 1;
		-- on met à jour seulement si la valeur n'existe pas encore ou est différente 
		IF prm_valeur IS DISTINCT FROM (SELECT pit_valeur FROM personne_info_text WHERE pit_id = pit_id_dernier) THEN
			-- On met la date de fin à la précédente valeur
			UPDATE personne_info_text SET pit_fin = CURRENT_TIMESTAMP WHERE pit_id = pit_id_dernier;
			-- puis on crée la nouvelle valeur
			INSERT INTO personne_info_text (pin_id, pit_valeur, pit_debut, uti_id) VALUES (the_pin_id, prm_valeur, CURRENT_TIMESTAMP, prm_uti_id);
		END IF;	
	ELSE
...
'Retourne les informations supplémentaires sur une thématique.';
...
'Modifie les informations supplémentaires sur une thématique.';

DROP FUNCTION IF EXISTS personne_liste_pour_varphoto(prm_token integer);
DROP TYPE IF EXISTS personne_liste_pour_varphoto;
CREATE TYPE personne_liste_pour_varphoto AS (
  per_id integer,
  per_name varchar,
  grp_id integer,
  grp_name varchar);

CREATE FUNCTION personne_liste_pour_varphoto(prm_token integer)
RETURNS SETOF personne_liste_pour_varphoto
LANGUAGE plpgsql
AS $$
BEGIN
  RETURN QUERY SELECT DISTINCT per_id, 
    personne_get_libelle(prm_token, per_id),
    grp_id, grp_nom
    FROM personne 
    ...
    AND ent_code = 'usager'
    AND personne_info_integer_get (prm_token, per_id, 'statut_usager') = 4;
END;
$$;
